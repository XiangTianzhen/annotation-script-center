...
- 当前阶段：柳州话脚本 `beta` 首版已接入。当前业务能力只在 `/app/editor/asr/` 生效，支持通过页内观察桥 + 多级回退获取当前音频签名 URL；页内桥会消费页面真实 `annotation/meta` 响应、顶层页面或同源 `xaudio` iframe 初始化阶段打印或请求到的真实音频 URL，扩展自身直连 meta 失败时会回退使用桥接 meta。助手区当前嵌入右侧 `全局标注` 卡片，保留原生 `Valid / Invalid` 在上方，但右侧已收口为连续紧凑信息区；`当前段 AI 推荐`、`填入当前推荐`、`生成画段建议`、`应用当前建议` 以及对应结果当前统一集中到中间 `普通话顺滑` 下方。当前段 AI 推荐严格按 `.xaudio_time` 的 `开始 / 结束` 裁剪当前段音频，浏览器端上传 1 小时 clip-cache 临时 URL 后再调用现有 AI recommend；缺少可信当前段时间时会直接阻断，不退回整段识别。基础设置里的两个提示屏蔽开关默认都开启，可分别拦截“您正在编辑该作业,不能打开新的Tab页”和“系统进入暂停状态”两类固定高层提示；前后端都只做“建议生成 + 人工确认”，不自动保存、不自动提交、不自动切下一条。
