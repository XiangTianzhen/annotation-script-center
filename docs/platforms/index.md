...
- 当前阶段：柳州话脚本 `beta` 已接入两阶段当前段识别链路。当前业务能力只在 `/app/editor/asr/` 生效，支持通过页内观察桥 + 多级回退获取当前音频签名 URL、`GET /httpapi/user/meta` 最小用户快照，以及页面真实 `annotation/*` 请求里的最小鉴权头；页内桥会消费页面真实 `annotation/meta` 响应、`user/meta` 响应、相关请求头，以及顶层页面或同源 `xaudio` iframe 初始化阶段打印或请求到的真实音频 URL，扩展自身直连 meta 失败时会回退使用桥接 meta。助手区当前嵌入右侧 `全局标注` 卡片，保留原生 `Valid / Invalid` 在上方；右侧当前只保留状态与逐行音频摘要，并优先插入 `.label_title_border2` 内容流，摘要固定显示 `文件 / 来源 / 当前第 N 段 / 当前段时间`。`是否有效（Valid or Not）` 下方当前作为独立同级 AI 工作区，优先挂到字段父容器 `div[data-v-fd55b986]` 内，集中承载 `当前段 AI 推荐 / 未填写补 Valid / 生成画段建议 / 应用当前建议`、当前画段建议结果和默认折叠的附加信息；字段区当前只保留两张最终结果卡：`修正后的柳州话文本` 回到 `标注文本`，`整理后的普通话文本` 回到 `普通话顺滑`，原始听音柳州话文本与 `特殊标签 / 需人工复核 / 备注 / AI 返回原始内容` 一并留在默认折叠但失败态也会保留的附加信息。附加信息区当前固定保留 6 项结构，缺失字段显示为空。当前中间 AI 区已新增 v1 `批量识别并自动填入`，并补上页内开关 `生成后自动应用当前建议`，默认开启；只有用户手动点击 `生成画段建议` 后才会触发自动应用，失败时保留 preview 不自动刷新。范围输入留空表示当前音频全部段，固定支持 `2-4 / 2,3,5 / 2-4,7`，前端固定并发 `5`、错峰 `50ms`，允许 AI 结果乱序返回；整批结束后只把成功段一次性构造成文本版 `save_increment` 写回平台，成功后刷新当前页一次。AI 设置当前改为 CVPC 专属 standalone 两阶段布局，只保留 `基础设置 / 听音 / 文本修正`，并在基础设置新增可切换单位的 `静音阈值`；当前画段建议链路改成“前端只传 `audioUrl + 阈值`，后端 Python 直接整音频分析”：后端通过 `miniaudio` 解码 mp3，按“静音 `>=0.4s`、阈值 `-27 dB`、支持切换 `% / Val`、前后补 `0.1s`、`30ms` 窗口、轻量平滑、`<=0.18s` 短尖峰桥接”生成整条音频建议段；当前不再依赖前端本地 `AudioContext` 画段分析。点击 `应用当前建议` 时，运行时会优先读取最新 `annotation/annos` 并构造平台 `save_increment` 请求，新增 `crypto.getRandomValues + timestamp` 的 `unique_id` 生成与请求体去重预检；如果本地或平台侧仍出现 `unique_id重复`，当前会保留 preview 并停止 DOM 回退。当前增量预览在普通直写失败场景下仍可回退同源 `xaudio` DOM 画段，整音频预览直写失败时保持 fail closed。当前段 AI 推荐严格按 `.xaudio_time` 的 `开始 / 结束` 裁剪当前段音频，浏览器端直接转成 Base64 `audioDataUrl` 后再调用 staged AI recommend；后端保留 `listen/refine` 两阶段命名，但 `listen` 只输出原始听音柳州话，`refine` 会结合 JSON 主词表优先生成的普通话草稿一次性输出两份最终文本，参考源 CSV 仅保留给人工整理和外部 AI 处理。缺少可信当前段时间时会直接阻断，不退回整段识别。基础设置里的两个提示屏蔽开关默认都开启，可分别拦截“您正在编辑该作业,不能打开新的Tab页”和“系统进入暂停状态”两类固定高层提示；当右侧 `.label_title_border2` 同时容纳 `柳州话脚本 Beta` 与 `AI 区` 所在字段分组时，当前会把 `Beta` 卡稳定排到 `AI 区` 上方。固定边界仍是：不自动提交、不自动切下一条，批量能力也只作用于当前音频 / 当前作业。当前 AI 调用已默认写脚本级 CSV，并在系统管理 `AI 请求记录` 导出列表中新增 `DataBaker CVPC 柳州话助手 AI 调用记录` 数据集。
...
- 2026-06-10 hotfix 补充：`批量识别状态` 当前直接显示固定并发 `5`；最终文本写回按成功段逐段对齐 latest rows，优先对齐 `uniqueId`，对不上时回退按锁定的 `selectionKey(start/end)` 近似匹配，不再要求 latest `annotation/annos` 全量 `unique_id` 列表完全一致。
