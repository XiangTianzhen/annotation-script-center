...
- 当前阶段：柳州话脚本 `beta` 已接入两阶段当前段识别链路。当前业务能力只在 `/app/editor/asr/` 生效，支持通过页内观察桥 + 多级回退获取当前音频签名 URL，并桥接 `GET /httpapi/user/meta` 的最小用户快照；页内桥会消费页面真实 `annotation/meta` 响应、`user/meta` 响应，以及顶层页面或同源 `xaudio` iframe 初始化阶段打印或请求到的真实音频 URL，扩展自身直连 meta 失败时会回退使用桥接 meta。助手区当前嵌入右侧 `全局标注` 卡片，保留原生 `Valid / Invalid` 在上方；右侧当前只保留状态与逐行音频摘要，并优先插入 `.label_title_border2` 内容流，摘要固定显示 `文件 / 来源 / 当前第 N 段 / 当前段时间`。`是否有效（Valid or Not）` 下方当前作为独立同级 AI 工作区，优先挂到字段父容器 `div[data-v-fd55b986]` 内，集中承载 `当前段 AI 推荐 / 未填写补 Valid / 生成画段建议 / 应用当前建议`、当前画段建议结果和默认折叠的附加信息；字段区当前只保留两张最终结果卡：`修正后的柳州话文本` 回到 `标注文本`，`整理后的普通话文本` 回到 `普通话顺滑`，原始听音柳州话文本与 `特殊标签 / 需人工复核 / 备注 / AI 返回原始内容` 一并留在默认折叠的附加信息。AI 设置当前改为 CVPC 专属 standalone 两阶段布局，只保留 `基础设置 / 听音 / 文本修正`，并在基础设置新增可切换单位的 `静音阈值`；当前画段建议链路改成“两段式本地预览 + 增量 DOM 应用”：默认先做“浏览器端静音检测 + 后端增量补切预览”，按“静音 `>=0.4s`、阈值 `-27 dB`、支持切换 `% / Val`、前后补 `0.1s`”生成建议；如果本地已检出静音但增量结果为空，会自动补一版 `whole-audio-rebuild-preview` 整条音频重切预览，且该 fallback 明确为 preview-only，不支持一键应用。浏览器端静音分析当前已补多声道能量、轻量平滑和 `<=0.18s` 短尖峰桥接，空预览也会提示是“未检出静音”“未命中现有段内部”还是“拆分后仍不足 2 段”。应用后只更新页面当前波形状态，仍需人工点击平台 `保存`。当前段 AI 推荐严格按 `.xaudio_time` 的 `开始 / 结束` 裁剪当前段音频，浏览器端直接转成 Base64 `audioDataUrl` 后再调用 staged AI recommend；后端保留 `listen/refine` 两阶段命名，但 `listen` 只输出原始听音柳州话，`refine` 会结合 JSON 主词表优先生成的普通话草稿一次性输出两份最终文本，参考源 CSV 仅保留给人工整理和外部 AI 处理。缺少可信当前段时间时会直接阻断，不退回整段识别。基础设置里的两个提示屏蔽开关默认都开启，可分别拦截“您正在编辑该作业,不能打开新的Tab页”和“系统进入暂停状态”两类固定高层提示；固定边界仍是：不自动保存、不自动提交、不自动切下一条。当前 AI 调用已默认写脚本级 CSV，并在系统管理 `AI 请求记录` 导出列表中新增 `DataBaker CVPC 柳州话助手 AI 调用记录` 数据集。
- 当前补采状态：`annotation/meta`、`annotation/annos`、模板字段和当前页 DOM 入口已纳入运行时；画段应用已能通过同源 `xaudio` DOM 交互写入页面当前波形状态，但仍不直连平台保存接口，也不自动触发提交流转。
