...
- 当前阶段：柳州话脚本 `beta` 已接入两阶段当前段识别链路。当前业务能力只在 `/app/editor/asr/` 生效，支持通过页内观察桥 + 多级回退获取当前音频签名 URL；页内桥会消费页面真实 `annotation/meta` 响应、顶层页面或同源 `xaudio` iframe 初始化阶段打印或请求到的真实音频 URL，扩展自身直连 meta 失败时会回退使用桥接 meta。助手区当前嵌入右侧 `全局标注` 卡片，保留原生 `Valid / Invalid` 在上方；右侧当前只保留状态、当前音频/当前段摘要和提示说明，并优先插入 `.label_title_border2` 内容流，摘要里会额外显示“当前第 N 段”。`是否有效（Valid or Not）` 下方当前作为独立同级 AI 工作区，集中承载 `当前段 AI 推荐 / 未填写补 Valid / 生成画段建议 / 应用当前建议`、当前画段建议结果和附加信息；`音频的柳州话文本 / 音频的普通话文本 / 修正后的柳州话文本` 三张结果卡则按字段分别回到 `标注文本` 与 `普通话顺滑` 块内。AI 设置当前改为 CVPC 专属 standalone 两阶段布局，只保留 `基础设置 / 听音 / 文本修正`，其中听音模型支持 `qwen3.5-omni-plus / qwen3.5-omni-flash`，文本修正模型支持 `qwen3.5-plus / qwen3.5-flash`。当前段 AI 推荐严格按 `.xaudio_time` 的 `开始 / 结束` 裁剪当前段音频，浏览器端直接转成 Base64 `audioDataUrl` 后再调用 staged AI recommend；整音频场景才继续使用公网 `audioUrl`。三张结果卡分别提供定向填入按钮。缺少可信当前段时间时会直接阻断，不退回整段识别。基础设置里的两个提示屏蔽开关默认都开启，可分别拦截“您正在编辑该作业,不能打开新的Tab页”和“系统进入暂停状态”两类固定高层提示；前后端都只做“建议生成 + 人工确认”，不自动保存、不自动提交、不自动切下一条。
