...
- 当前阶段：柳州话脚本 `beta` 已接入两阶段当前段识别链路。当前业务能力只在 `/app/editor/asr/` 生效，支持通过页内观察桥 + 多级回退获取当前音频签名 URL、`GET /httpapi/user/meta` 最小用户快照，以及页面真实 `annotation/*` 请求里的最小鉴权头；页内桥会消费页面真实 `annotation/meta` 响应、`user/meta` 响应、相关请求头，以及顶层页面或同源 `xaudio` iframe 初始化阶段打印或请求到的真实音频 URL，扩展自身直连 meta 失败时会回退使用桥接 meta。助手区当前嵌入右侧 `全局标注` 卡片，保留原生 `Valid / Invalid` 在上方；右侧当前只保留状态与逐行音频摘要，并优先插入 `.label_title_border2` 内容流，摘要固定显示 `文件 / 来源 / 当前第 N 段 / 当前段时间`。`是否有效（Valid or Not）` 下方当前作为独立同级 `柳州话 AI 识别助手`，固定按 `当前段识别 / 批量识别 / 分段建议 / AI信息` 4 个分区展示；字段区当前只保留两张最终结果卡：`修正后的柳州话文本` 回到 `标注文本`，`整理后的普通话文本` 回到 `普通话顺滑`。`AI信息` 区当前默认折叠但始终保留，失败态也不会消失；内部固定顺序展示 `听音识别 / 文本修正 / 音频听出的柳州话文本 / 特殊标签 / 需人工复核 / 备注 / AI 返回原始内容`，其中两段阶段信息只显示 `模型 / 输入 / 输出`，不再展示 `总输入 / 总输出 / 总计`。当前中间 AI 区已新增 v1 `批量识别并自动填入`，并补上页内开关 `生成后自动应用分段建议`，默认开启；只有用户手动点击 `生成分段建议` 后才会触发自动应用，失败时保留 preview 不自动刷新。批量入口当前默认显示 `全部 + 段号选择框`，默认全选当前音频全部段，支持点击单段与拖动连续选择；前端固定并发 `5`、错峰 `50ms`，允许 AI 结果乱序返回；整批结束后只把成功段一次性构造成文本版 `save_increment` 写回平台，成功后刷新当前页一次。AI 设置当前改为 CVPC 专属 standalone 两阶段布局，只保留 `基础设置 / 听音 / 文本修正`，并在基础设置新增可切换单位的 `静音阈值`；当前分段建议链路改成“前端只传 `audioUrl + 阈值`，后端 Python 直接整音频分析”：后端通过 `miniaudio` 解码 mp3，按“静音 `>=0.4s`、阈值 `-27 dB`、支持切换 `% / Val`、前后补 `0.1s`、`30ms` 窗口、轻量平滑、`<=0.18s` 短尖峰桥接”生成整条音频建议段；当前不再依赖前端本地 `AudioContext` 分段分析。点击 `应用分段建议` 时，运行时会优先读取最新 `annotation/annos` 并构造平台 `save_increment` 请求，新增 `crypto.getRandomValues + timestamp` 的 `unique_id` 生成与请求体去重预检；如果本地或平台侧仍出现 `unique_id重复`，当前会保留 preview 并停止 DOM 回退。当前增量预览在普通直写失败场景下仍可回退同源 `xaudio` DOM 分段，整音频预览直写失败时保持 fail closed。当前段 AI 推荐严格按 `.xaudio_time` 的 `开始 / 结束` 裁剪当前段音频，浏览器端直接转成 Base64 `audioDataUrl` 后再调用 staged AI recommend；后端保留 `listen/refine` 两阶段命名，但 `listen` 只输出原始听音柳州话，`refine` 会结合 JSON 主词表优先生成的普通话草稿一次性输出两份最终文本，参考源 CSV 仅保留给人工整理和外部 AI 处理。缺少可信当前段时间时会直接阻断，不退回整段识别。基础设置里的两个提示屏蔽开关默认都开启，可分别拦截“您正在编辑该作业,不能打开新的Tab页”和“系统进入暂停状态”两类固定高层提示；当右侧 `.label_title_border2` 同时容纳 `柳州话脚本 Beta` 与 `AI 区` 所在字段分组时，当前会把 `Beta` 卡稳定排到 `AI 区` 上方。固定边界仍是：不自动提交、不自动切下一条，批量能力也只作用于当前音频 / 当前作业。当前 AI 调用已默认写脚本级 CSV，并在系统管理 `AI 请求记录` 导出列表中新增 `DataBaker CVPC 柳州话助手 AI 调用记录` 数据集。
- 2026-06-10 hotfix：批量入口当前复用 `当前段 AI 推荐` 同款橙色按钮；写回前不再重复重抓整套上下文，只校验 live `selectedEntryName`，并在识别当前文件名时避开左侧 `音频列表` 中的 `.mp3` 文本。旧版 Network 里看到的多条相同 `GET /httpapi/annotation/annos` 属于状态读取，不是多次保存。
- 2026-06-10 hotfix 补充：`批量识别状态` 当前直接显示固定并发 `5`；最终文本写回按成功段逐段对齐 latest rows，优先对齐 `uniqueId`，对不上时回退按锁定的 `selectionKey(start/end)` 近似匹配，不再要求 latest `annotation/annos` 全量 `unique_id` 列表完全一致；目标段缺少文本 attr 时会复用同音频其他段或模板字段定义补齐后再写回。
...
- 客家话助手补充（2026-06-09）：`#/asrmark` 已新增当前页临时“全自动”模式，固定执行 `等待加载 -> AI识别 -> 填入 -> 提交 -> 等待下一条`；默认关闭，通过可录制快捷键 `开启/关闭全自动` 控制，AI 四项都正确时会直接继续提交；失败即停、手动关闭立即停止，`#/asrmarkCheck` 不参与自动提交。
