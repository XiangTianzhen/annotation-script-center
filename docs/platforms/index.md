...
- 2026-06-11 带标签当前段填入兜底补强：当前段 `标注文本` 带标签写入现在会优先走“编辑器内顺序插入文本 + 页面原生标签按钮回放”，只有这条路不可用时才回退旧的 `MAIN world` 结构化写入桥，进一步贴近人工操作路径。
