...
- 返修列表批量导入严格串行处理当前响应中的最多 10 条；真正新建成功且安全映射保存完成后，才等待 1 秒并发起下一条，最后一条成功后不额外等待。后端确认 `replayed/existing` 时计入“复用”并立即处理下一条，不重复创建也不执行间隔；任意单条失败立即停止整批。运行中禁止重复开始，并显示总数、已处理、成功、复用、跳过、失败和当前 ItemID。用户停止后不再发起下一条，已发出的单条允许结算；页码、任务或路由范围变化时自动停止。
