...
- 自动押后只驱动真实可见 DOM：按钮文本必须精确为“押后”，不得匹配“提交”；只接受唯一、可见、标题精确为“押后原因”的弹层，在其唯一 textarea 写入 `1` 并只点击同一按钮组的“确定”。每个阶段最多等待 20 秒；仅 `AVAILABLE`（待领取）与 `SUBMITTED`（待审核领取）可继续，缺失或多重按钮/弹层、原因写入失败、其他状态、导入/刷新失败或确认后未验证下一题均立即停止，不重试、不跳过、不领取、不保存、不提交且不调用 AIDP API。确认前的停止只点击同一弹层“取消”；确认已发送后不再执行任何下一步。下一题完成初始化但没有可用“押后”按钮时正常报告“已无可押后数据”。
