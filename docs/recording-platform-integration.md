...
- “添加数据”先按当前任务编号与 AIDP ItemID 读取本地映射，并在创建流程内部再次检查以阻断连续点击竞态。映射已存在时只使用浏览器原生弹窗提示现有 itemCode，不发创建请求或结果请求，不重绘状态、文本或音频；本地映射丢失时仍由服务器幂等重放兜底，HTTP 200 响应会重建映射并提示条目已存在，HTTP 201 才表示本次新建。
...
- “当前录音平台结果”不显示内部关联用的 sourceItemId，始终显示“录音条目”；未导入时显示“还未导入该条目”，点击刷新只显示“暂无可刷新结果”且不请求服务器。首次创建后立即使用创建响应中的真实 itemCode 和 status，只有后续手动刷新才请求最新结果。仅完成后展示文本和/或结果音频，不展示结果视频，不写入 AIDP textarea、画段、暂存或提交接口。
