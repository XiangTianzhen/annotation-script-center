...
`platform-resources/` 保存四个已接入平台的稳定参考资料、六脚本专属后端，以及所有已接入脚本共用的统一后端能力。
...
`registry.js` 注册六个已接入脚本和当前管理员能力。各脚本路由继续由对应 `backend/index.js` 导出 `registerRoutes(router, options)`，统一入口负责组合，不把脚本业务复制到公共后端。
