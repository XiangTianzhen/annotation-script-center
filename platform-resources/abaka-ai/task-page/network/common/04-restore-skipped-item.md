...
1. 跳转 `/items?taskId={taskId}&dm=skipped&itemId={itemId}&nodeId={nodeId}&selectIds={itemId}`。
2. `POST /api/v2/item/get-item-history`
3. `POST /api/v2//item/check-operate-item-permission`
4. `POST /api/v2/item/get-item-info`
5. `POST /api/v2/item/work`
6. `POST /api/v2/label/find-labels`
7. `POST /api/v2/label/find-issues`
8. `POST /api/v2/label/find-label-records`
9. `POST /api/v2/label/get-ai-check-result`
10. `POST /api/v2/item/find-invalidate-frame/`
11. `POST /api/v2/item/sampling/get-frames-data`
12. `POST /api/v2/item/find-items-base-info`
