...
| 恢复已跳过 | `/task-v2/data-item?dm=skipped` | `state-change-test` / `danger-confirm` | `network/common/04-restore-skipped-item.md` | 选中后 `Label: 1` 调用 `receive-item` 并进入标注页 | 回到 `LABELING / WORKING` | 必须人工确认，不自动领取 |
| 恢复已放弃 | `/task-v2/data-item?dm=abandoned` | `state-change-test` / `danger-confirm` | `network/common/05-restore-dropped-item.md` | `Recovery` 确认后调用 `/api/v2/item/recover-item` | 已恢复，Dropped 列表移出 | 必须人工确认，不自动恢复 |
| 标注内审只读观察 | `/task-v2/data-item?role={reviewRoleId}` | `safe-read` | `network/common/07-review-role-no-submit.md` | 只观察列表、Tab、View 初始化 | 不涉及 | 禁止自动提交 / 通过 / 驳回 |
...
| 送审 | 送审 | Submit | 标注页 | 是 | 是 | `network/11-submit-review.md`、`network/common/06-label-submit-success.md` | 二次确认，禁止自动送审 |
