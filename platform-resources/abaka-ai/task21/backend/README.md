...
- `health/defaults` 当前已补齐公共 jobs / runtime 元信息，默认链路为 `POST /jobs` + 轮询 `GET /jobs/:jobId`，同步 `analyze` 只保留兼容 / 调试入口；默认 `jobs.runningTimeoutMs=60000`、`jobs.failedRetentionMs=60000`、`queue.defaultModelPool.maxSize=9999`、`queue.defaultModelPool.pendingTimeoutMs=0`。
- 日志文件：
