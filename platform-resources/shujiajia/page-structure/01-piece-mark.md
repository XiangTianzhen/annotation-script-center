...
| 助手控制区 | `.form-tabs .tabs-container` | 中 | 追加在有效性表单之后，不移动已有子节点 |
| 助手结果区 | `.operate-container .transfer` | 中 | 追加在原生 `.transfer-container` 之后，可折叠 |
...
辅助面板只向 iframe 内两个已确认区域追加自有节点：控制区作为 `.tabs-container` 最后一个子节点，位于“是否有效”下方；结果区作为 `.transfer` 最后一个子节点，位于原生 `.transfer-container` 下方。两个区域使用平台现有纵向滚动，不修改父容器宽高、Grid 或 Flex，也不移动原生节点；挂载必须幂等，并在 Vue 局部重渲染后重新定位目标容器并恢复自身节点。
