...
| Peaks.js 波形与加载标记 | `.audio-peaks .waveform`，内部为 `#peaks-waveform` Canvas | 中 | 切题自动划段要求出现不同于上一题的唯一、已连接、可见且尺寸有效的新节点；画段前仍重新读取尺寸，段落本身不提供 Wavesurfer region DOM |
...
- 切题上下文可能早于新波形节点创建；旧页面波形即使仍可读，也不能作为新条目自动划段的就绪依据。首次条目会持续观察并记录稍后创建的波形节点作为后续基线。
...
- 整段划分按平台操作指引通过可信 `Shift + 拖拽` 完成；iframe 坐标只在顶层换算后发送到受路由与视口限制的扩展后台。切题自动入口必须先确认本题已创建不同于上一题的唯一可用 `.audio-peaks .waveform`，再等待音频时长与波形宽度；10 秒未确认时取消且不降级。只有零段落允许执行，结果必须通过表格单段数量、区域文字和两像素边界校验；超出容差时保留段落并提示人工检查，不自动 Delete，也不直接写 annotation 对象或调用平台写接口。
