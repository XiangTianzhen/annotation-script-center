...
| 助手结果区 | `[data-asc-shujiajia-luzhou-drawer]`；宿主优先为 `.operate-container .transfer`，缺失时使用 `.paragraph` 前的扩展专属占位区 | 中 | 始终显示并保持幂等挂载；不再作为自动划段就绪信号 |
| Peaks.js 波形 | `.audio-peaks .waveform`，内部为 `#peaks-waveform` Canvas | 中 | 自动延迟到期后由现有整段动作读取尺寸和音频时长；段落本身不提供 Wavesurfer region DOM |
...
- 外层标注页稳定路由为 `https://www.shujiajia.com/workbench/piece/mark.html`；自动划段从 iframe 的 `document.referrer` 校验该路由、非空 `taskId` 和 `executeClass=TAG_PIECE`。同一任务内 URL 不随音频切换，因此新条目仍以 execute 响应形成的 `taskId:dataId` 判断。
...
- 整段划分按平台操作指引通过可信 `Shift + 拖拽` 完成；iframe 坐标只在顶层换算后发送到受路由与视口限制的扩展后台。自动入口在合法外层 URL 下由新 `taskId:dataId` 启动固定延迟：页面会话首个条目默认 2500ms，后续条目默认 500ms，两项可配置范围均为 500–5000ms；到期后只调用一次现有动作，不做 DOM 或网络就绪轮询。只有零段落且波形尺寸、音频时长有效时允许执行，结果必须通过表格单段数量、区域文字和两像素边界校验；超出容差时保留段落并提示人工检查，不自动 Delete，也不直接写 annotation 对象或调用平台写接口。
