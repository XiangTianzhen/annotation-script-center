...
| 提交进入下一条 | `button.submit` | 高 | 后续首版的提交与切题入口 |
...
| 波形区域 | 可见波形、进度和播放控件的组合语义 | 中 | 分段交互事件尚待确认 |

## 动态区域 / 重渲染风险

- `#bdIframe` 会随切题或模板初始化重新加载，旧 `contentDocument` 和节点引用会失效。
- 分段表格增删段落后可能整表重绘，行索引不应作为段落身份。
- 输入、选段和波形操作可能触发平台事件链与自动暂存，直接赋值无法代表真实写入成功。
- 动态 ID、框架生成属性和临时 class 不作为稳定契约。
- 外层按钮的禁用、隐藏或加载态必须原样遵守，不得绕过。

## 可挂载点建议

后续接入时，辅助面板建议挂载在外层任务操作区附近的独立容器，不插入 iframe 的分段表格或波形内部。挂载必须幂等，并在外层重绘或 iframe 重载后重新校验。

当前阶段不创建挂载代码，也不显示任何功能按钮。
...
| 分段表格、转写、有效性 | annotation JSON 与暂存链路 | 页面事件更新 `markResult.paragraphs[]`，平台自动暂存 |
...
- 分段调整通过真实 DOM 和平台事件完成，由平台原生逻辑自动暂存；不直接写 annotation 对象或调用平台写接口。
- 最终提交使用唯一、可见、可用的原生按钮；不直接调用提交 API。
- 音频真实请求、波形分段事件、选中段标记和暂存完成信号尚未确认。
