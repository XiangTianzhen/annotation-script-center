...
- 整段划分按平台操作指引通过可信 `Shift + 拖拽` 完成；iframe 坐标只在顶层换算后发送到受路由与视口限制的扩展后台。自动入口在合法外层 URL 下由新 `taskId:dataId` 启动固定延迟，默认 2500ms；到期后只调用一次现有动作，不做 DOM 或网络就绪轮询。只有零段落且波形尺寸、音频时长有效时允许执行，结果必须通过表格单段数量、区域文字和两像素边界校验；超出容差时保留段落并提示人工检查，不自动 Delete，也不直接写 annotation 对象或调用平台写接口。
