...
- 切题自动划段以 `taskId:dataId` 为上下文，首次条目记录平台原生 `.audio-peaks .waveform` 节点；后续切题只接受与上一题不同、唯一、已连接、可见且尺寸有效的新波形节点，再等待音频时长与波形宽度就绪。DOM 只通过 `MutationObserver` 被动观察，不读取 Resource Timing，也不主动请求字体、WASM 或 favicon。等待 10 秒仍未确认新音频区域时取消本题且不降级，同一条目在当前页面会话最多尝试一次。手动画段继续只使用原有波形与零段落检查。
