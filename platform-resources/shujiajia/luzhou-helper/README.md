...
- 自动划段以新的 `taskId:dataId` 为触发信号，首题与后续切题均执行；iframe 的 `document.referrer` 必须精确指向 HTTPS `/workbench/piece/mark.html`，带非空 `taskId` 且 `executeClass=TAG_PIECE`。通过页面门禁后按 Options 固定延迟（默认 500ms，范围 0–50000ms，`0ms` 表示下一轮异步计时后立即尝试）单次调用现有整段动作，不再等待网络资源、结果框重挂或波形轮询，也不重试。schema 42 自动把旧 schema 中的原默认 2500ms 迁移为 500ms，其他合法自定义值不变。延迟到期仍会复核 URL、条目身份及开关；切题、关闭开关、禁用脚本或停止运行时取消旧计时。手动画段继续只使用原有波形与零段落检查，不受 URL 延迟门禁影响。
