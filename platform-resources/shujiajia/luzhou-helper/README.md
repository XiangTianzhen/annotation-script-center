...
- 切题自动划段以 `taskId:dataId` 为上下文，首次条目记录扩展 AI 识别结果框的挂载代次；后续切题只接受挂载代次增加且结果框唯一、已连接、未隐藏、可见、尺寸有效的状态，再等待音频时长与波形宽度就绪。重复幂等挂载不增加代次；DOM 只通过 `MutationObserver` 被动观察，不读取 Resource Timing，也不主动请求字体、WASM 或 favicon。等待 10 秒仍未确认结果框重挂时取消本题且不降级，同一条目在当前页面会话最多尝试一次。手动画段继续只使用原有波形与零段落检查。
