...
- 默认 Prompt 按原音逐字转写，要求内部复听，禁止同义替换、普通话化、方言化、补写和润色；保留方言词、重复、口吃、语气词、数字实际读法及可确认的专名。模型只返回最终纯文本，不返回 JSON、分析或不确定性标记。
...
- 自动划段以新的 `taskId:dataId` 为触发信号，页面会话首个条目默认延迟 2500ms，后续条目默认延迟 500ms，两项范围均为 500–5000ms；即使首个上下文因 URL 不合法或开关关闭而未安排，也不会把下一条误判为首次。iframe 的 `document.referrer` 必须精确指向 HTTPS `/workbench/piece/mark.html`，带非空 `taskId` 且 `executeClass=TAG_PIECE`。通过页面门禁后单次调用现有整段动作，不再等待网络资源、结果框重挂或波形轮询，也不重试。schema 43 为旧设置补入首次延迟，并把有效的旧单一延迟迁到后续条目延迟，越界时回退 500ms。延迟到期仍会复核 URL、条目身份及开关；切题、关闭开关、禁用脚本或停止运行时取消旧计时。手动画段继续只使用原有波形与零段落检查，不受 URL 延迟门禁影响。
- MAIN world 观察器不替换平台原生 XHR `send`；只在 `open` 后以私有 `WeakMap` 记录 URL、方法和 `loadstart` 时的条目上下文，并在 `load` 时处理 execute、tempsave 或音频响应。其他平台请求不进入扩展业务处理。
