...
- 切题自动划段以 `taskId:dataId` 为上下文，只接受当前 iframe 在本次切题后成功完成的 `template.shujiajia.com/**/multi-audio4/fonts/element-icons.woff` Resource Timing 记录，再等待波形就绪；切题前旧记录不复用，版本目录与查询参数不参与匹配。等待 10 秒仍未确认成功时取消本题且不降级，同一条目在当前页面会话最多尝试一次。手动画段继续只使用原有波形与零段落检查。
