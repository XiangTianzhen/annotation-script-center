...
function main() {
  ...
  try {
    ...
    console.error(
      "[convert-hakka-lexicon] 未找到 xlsx 依赖，无法自动转换。请先手动将 Excel 另存为 UTF-8 CSV。"
    );
    process.exit(1);
  }
  ...
  if (!sheetName) {
    console.error("[convert-hakka-lexicon] Excel 中没有可用工作表。");
    process.exit(1);
  }
  ...
  console.info("[convert-hakka-lexicon] 转换完成:", outputFile);
}
