...
- 已接入扩展运行时代码：`extension/sites/magic-data/hakka-helper/`（共享模块位于 `extension/sites/magic-data/shared/`）。
- 已接入页面内质检区：`#/asrmark` 右侧“句子列表”下方固定卡片。
- 已接入后端接口：`GET/POST /api/magic-data/hakka-helper/ai/review-current*`（兼容旧 `/api/magic-data/annotator/ai/*`）。
...
- 第一版 AI 调试接口已落地：
  ...
  - 兼容旧路径：`/api/magic-data/annotator/ai/*`
- 第一版不新增业务写接口。
