...
- 当前状态：已接入运行时、分段建议、单段识别直填输入框、批量识别、AI 调用记录与平台暂存写回
  ...
  - AIDP 设置页局部 select 已重构为 AIDP 专属自定义下拉，展开菜单统一挂到脚本详情页自己的 `aidp-ui-layer` 局部门户层，避免被卡片裁切；右侧 `AI 设置` 基础控制固定按“先开关、后请求超时时间（秒）”排布
  - Prompt 与 `stop sequences` 新增按实际可见行计算的行号 gutter，并提供仅当前页面会话生效的竖向行数滑块；默认 `Prompt=10`、`stop sequences=1`
