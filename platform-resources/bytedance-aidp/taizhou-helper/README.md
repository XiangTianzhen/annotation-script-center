...
- 台州话辅助面板可由用户显式启动“全自动导入并押后”，这是仅限当前详情页和当前任务的 DOM 自动化例外，不扩散到其他脚本、Options、后端或 storage。每轮先走既有导入/刷新并只接受 `AVAILABLE`（待领取）或 `SUBMITTED`（待审核领取），再只点击可见、启用、文本精确为“押后”的原生按钮；唯一可见“押后原因”弹层内的 textarea 必须写为 `1`，确认按钮必须在同一弹层按钮组。绝不匹配或点击“提交”，不调用 AIDP 接口；任何不唯一、状态不符、20 秒超时或未验证 Receive ItemID 切换都会停止，不重试、不跳过。确认前停止会取消弹层，确认后停止不继续下一步。
