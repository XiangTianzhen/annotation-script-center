...
- 不写 `ms`，不调用保存、提交或切题接口。
- 录音导入只观察 Search Item 的最小安全字段；Options 可见任务编号与服务器 `allowedTaskCodes` 双重匹配后，浏览器把参考文字及原始公网 HTTPS 音视频 URL 交给脚本中心专用端点，后端固定附加 `BYTEDANCE_AIDP + ItemID` 来源绑定。扩展不下载或上传媒体；机器 Key、AIDP Cookie、Authorization 与 Session 不进入扩展存储或服务器请求。
