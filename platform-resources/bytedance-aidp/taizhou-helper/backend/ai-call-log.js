...
function getLogDir() {
  const customDir = normalizeText(process.env.BYTEDANCE_AIDP_TAIZHOU_AI_CALL_LOG_DIR);
...
const aiCallLogger = createAiCallLogger({
  ...
  buildExtendedRow(context) {
    ...
    return {
      ...
      omniModel: normalizeText(models.omniModel || input.aiOmni?.model || input.omniModel),
