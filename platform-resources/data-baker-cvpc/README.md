...
- `liuzhou-helper/`：已补脚本级规则资产、独立后端接口和扩展运行时 README；当前右侧只保留嵌入 `全局标注` 卡片的紧凑状态区，并优先插入 `.label_title_border2` 内容流；中间 `普通话顺滑` 下方统一承载画段建议和三结果 AI 推荐卡；当前段 AI 推荐改为“浏览器裁剪当前段 -> clip-cache 临时 URL -> staged AI recommend”
