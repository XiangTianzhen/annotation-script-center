...
- 当前页工具面板：
  ...
  - 若最终建议只剩单独出现的任意单个标签与标点，当前段定向填入、整卡填入和批量写回都会改写为 `Invalid + <Meaningless> + 空普通话顺滑` 预设：`标注文本` 写结构化 `<Meaningless>`，`普通话顺滑` 清空
  - 当前段识别成功后默认会自动整套填入页面；自动填入同样复用上述 `<Meaningless>` 预设，因此像 `#um。/#hmm。/#ah。/#eh。/<SPK/>。/<NPS/>。` 这类单独标签结果都会直接落成 `Invalid + <Meaningless> + 空普通话顺滑`
  - 命中上述 `<Meaningless>` 预设时，字段结果卡与 `AI信息` 里的 `柳州话修正参考 / 普通话顺滑参考` 当前也会同步显示最终应用结果，不再继续显示原始 `#um / 嗯。` 或 `<SPK/> / 。`；原始 AI 返回仍只保留在 `AI 返回原始内容`
  ...
  - 当前段所有写入动作当前都会先按柳州话推荐标签校验 `是否有效（Valid or Not）`：`<SPK/> / <NPS/> / #um / #hmm / #ah / #eh` 需要 `Valid`，`<Unintelligible> / <Meaningless> / <Silence>` 需要 `Invalid`
  - 若同一条推荐里同时出现有效标签和无效标签，或无效标签与语义正文并存，当前会直接判为“标签内容冲突”，停止本次当前段写入，避免把坏结果落页
  - `填入普通话顺滑` 当前也会复用同一条柳州话推荐的标签作为有效性参考，不会因为目标字段本身无标签而跳过 `Valid / Invalid` 联动
  - 当前新增页内持久化开关 `标签与有效性不一致时直接修正`，默认开启；开启时会先自动切换 `Valid / Invalid` 再写入，并在状态区提示已自动修正；关闭后会改用原生确认框，点“否”则整次写入取消
  - 当前段带标签填入时，主路径会优先走“编辑器内顺序插入文本 + 页面原生标签按钮回放”；空结构化值 `[]` 当前会按真正空文本处理，不再把 `"[]"` 误判成有内容；若原生回放任一步未通过校验，会先恢复旧快照，避免出现“点填入后输入框被清空”；只有这条路不可用时，才回退到 `MAIN world` 结构化状态桥 + `modelvalue + ProseMirror` 可视自愈兜底，降低 `<Meaningless>`、`#eh` 一类 chip 被页面下一轮渲染回滚的概率
  - `当前段识别` 区当前新增页内持久化开关 `识别完成后自动填入`，默认开启；关闭后识别只展示结果，不自动写回页面
  - 批量写回当前也会按同一套标签规则自动修正 `Valid / Invalid`；如果某段标签内容冲突，只跳过该段并记录失败，其余成功段继续保存
  ...
  - 中间 AI 区当前新增 `批量识别并自动填入`（v1）：
    ...
    - `批量识别状态` 当前会直接显示固定并发数，并实时累计 `批量输入Token / 批量输出Token / 批量总Token / 批量预估人民币`
...
- options / AI 设置：
  ...
  - `基础设置` 当前新增默认开关 `识别完成后自动填入`，用于控制当前段识别成功后是否立即整套填入页面
...
- `POST /api/data-baker-cvpc/liuzhou-helper/ai/recommend`
  ...
  - 标签归一化：
    ...
    - 若最终柳州话 token 只剩单独出现的任意单个标签与标点，前端应用层会按 `<Meaningless>` 预设落页：切为 `Invalid`、`标注文本` 写 `<Meaningless>`、`普通话顺滑` 留空
    - 前端写入层会继续根据最终柳州话 token 自动推断 `requiredValidity = valid | invalid | conflict | none`；当前段遇到 `conflict` 会停止写入，批量则跳过冲突段并继续写回其它成功段
...
6. 批量写回成功后确认页面只刷新一次；刷新后复核 `标注文本 / 普通话顺滑` 已更新；常规文本段的 `Valid / Invalid` 不应被改动，只有“最终建议仅为单独出现的任意单个标签”的段会自动切成 `Invalid + <Meaningless>` 预设；整个流程也不应自动提交或切下一条。
