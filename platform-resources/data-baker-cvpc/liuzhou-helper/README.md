...
2. 在中间 AI 区确认看到范围输入框、`批量识别并填入`、`停止批量` 和批量状态区。
3. 留空范围执行一次，确认当前音频全部段进入批量状态区，并实时显示 `总数 / 已发起 / 进行中 / 已成功 / 已失败 / 当前段 / 失败清单`。
4. 分别用 `2-4`、`2,3,5`、`2-4,7` 验证只处理指定段；非法 token、越界号和空结果应在前端直接拦截。
5. 批量中点击 `停止批量`，确认不会继续发新请求，但已发起段会自然完成；最终只保存成功段。
6. 批量写回成功后确认页面只刷新一次；刷新后复核 `标注文本 / 普通话顺滑` 已更新，`Valid / Invalid` 未改动，也没有自动提交或切下一条。
