...
- 当前页工具面板：
  ...
  - 当前段识别成功后默认会自动整套填入页面；自动填入同样复用上述 `<Meaningless>` 预设，因此像 `#um。/#hmm。/#ah。/#eh。` 这类单独语气词结果会直接落成 `Invalid + <Meaningless> + 空普通话顺滑`
  - 命中上述 `<Meaningless>` 预设时，字段结果卡与 `AI信息` 里的 `柳州话修正参考 / 普通话顺滑参考` 当前也会同步显示最终应用结果，不再继续显示原始 `#um / 嗯。`；原始 AI 返回仍只保留在 `AI 返回原始内容`
