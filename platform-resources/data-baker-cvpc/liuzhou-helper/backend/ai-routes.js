...
function registerAiRoutes(router) {
  router.get(AI_BASE_PATH + "/health", function (_context) {
    sendJson(_context.response, 200, aiService.createHealthPayload(createAssetContext()));
  });

  router.get(AI_BASE_PATH + "/defaults", function (_context) {
    sendJson(_context.response, 200, aiService.createDefaultsPayload());
  });
