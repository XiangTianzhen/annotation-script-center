...
- 左侧句子列表上方 `filter-screen`（“全选/批量判定”同一行）新增“AI填入合格项”：按钮挂载在“批量判定”右侧。点击后先刷新当前页 `queryCollectStatementByCondtion`，仅处理 `statusName=质检合格` 的下一条，自动选中并填入 AI 推荐文本；每次只处理单条。
...
- “AI填入合格项”不会自动保存、自动提交、自动批量流转，也不会点击左侧 checkbox；`质检不合格` 与 `未质检` 会直接跳过。
