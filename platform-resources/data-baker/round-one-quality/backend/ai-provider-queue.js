...
function isRetryableProviderError(error) {
  ...
  return error.code === "provider-http-error" && Number(error.statusCode) === 429;
}
  ...
  async executeWithRetry(task, settings, queueMeta) {
    ...
    for (let attempt = 0; attempt <= settings.retryMax; attempt += 1) {
      try {
        ...
        lastError = error;
        if (!isRetryableProviderError(error) || attempt >= settings.retryMax) {
          break;
        }
    ...
    if (isRetryableProviderError(lastError)) {
      const wrapped = new Error("上游模型限流，后端已重试仍失败，请稍后重试。");
      wrapped.code = "provider-rate-limited";
      wrapped.statusCode = 429;
      wrapped.providerStatus = 429;
      wrapped.summary = String(lastError.summary || lastError.message || "").slice(0, 200);
      throw wrapped;
    }
    throw lastError;
  }
...
async function enqueueProviderTask(groupName, task) {
  const queue = getOrCreateQueue(groupName);
