...
CSV 下载地址必须带 `supplier`：

```text
http://127.0.0.1:3333/api/alibaba-labelx/asr-judgement/statistics/download?supplier=棋燊
```
...
- `file-store.js`：默认写入 `statistics-data/suppliers/<供应商>/statistics-merged.csv`，必要时可兼容读取历史根级 CSV 和旧 `statistics-rows.json`。
...
- `GET /api/alibaba-labelx/asr-judgement/statistics/download?supplier=<供应商>`：下载指定供应商的 `statistics-merged.csv`；未传 `supplier` 返回 `400`。
...
- `suppliers/<供应商>/statistics-merged.csv`：指定供应商的最终合并 CSV。
- `statistics-merged.csv`：历史根级 CSV，仅作为迁移输入读取，不再写回。
