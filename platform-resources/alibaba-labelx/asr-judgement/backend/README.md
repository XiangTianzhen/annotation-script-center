...
- `GET /api/alibaba-labelx/asr-judgement/ai/health` 与 `GET /api/alibaba-labelx/asr-judgement/ai/defaults` 当前已补齐公共 jobs / runtime 元信息，默认链路为 `POST /jobs` + 轮询 `GET /jobs/:jobId`；默认 `jobs.runningTimeoutMs=60000`、`jobs.failedRetentionMs=60000`、`queue.defaultModelPool.maxSize=9999`、`queue.defaultModelPool.pendingTimeoutMs=0`。
