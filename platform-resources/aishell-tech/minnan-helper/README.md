...
- 当前默认链路已经切到“短请求创建 job + 轮询结果”；同步 `POST /recommend` 只保留兼容 / 调试入口，不引入 SSE 或 WebSocket。
- 共享 model pool 与公共 job store 默认容量都已提升到 `9999`；模型池待启动超时当前默认关闭（等效无限等待）。若后续重新启用排队超时，失败记录仍默认保留 `60000ms` 供轮询读取。
...
- 前端批量链路当前为“AI 并发请求 + 页面串行保存”：
  ...
  - AI 结果谁先返回，谁先进入保存队列。
  - 真正写页面时不仅派发 `input`/`change`，还会触发 `blur`，且会进行 5 次填入校验重试以避免 Vue 异步渲染导致的输入重置；之后会按条目编号与文件名后缀重新匹配左侧列表，再触发平台真实“保存”按钮。
  - 右侧当前条校验会优先从 `.fileName-line` 整行提取 `编号: 文件名.wav`，兼容平台把编号单独放在首个 `span`、文件名和工具按钮共用一行的结构。
  - 保存成功优先以页面 `保存成功!` 提示为准，且点击保存后强制等待 250ms 以防瞬时竞态误判；若提示未及时出现，再回退检查 `getShortMark / packageItemList` 的保存结果。
