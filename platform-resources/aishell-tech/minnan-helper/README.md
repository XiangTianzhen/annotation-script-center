...
- 当前只保留 `audio_first_reference`（音频优先，文本参考）策略：
  ...
  - 比较阶段当前改成 `heardText + lexiconCandidateText + pageText` 三文本对照：先单独调用一次文本模型，结合 `minnan-lexicon.csv` 把 `pageText` 转成标准闽南语候选，再只重点检查与 `heardText` 的差异项。
...
- 前端 UI 口径当前固定为“嵌入式推荐卡片 + 原生按钮注入”：
  ...
  - 当前识别结果区会显示原始文本、由独立文本模型结合词表生成的词表转写文本、听音文本、推荐文本，以及“听音文本 vs 词表转写文本”的差异高亮。
