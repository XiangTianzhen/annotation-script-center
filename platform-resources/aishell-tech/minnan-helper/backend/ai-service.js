...
function createDataBakerRequest(normalizedRequest) {
  ...
  const aiOptions = normalizeAiOptions(request.aiOptions);
  ...
  const recognitionMode = normalizeModelText(request.recognitionMode);
  if (recognitionMode) {
    dataBakerRequest.recognitionMode = recognitionMode;
    dataBakerRequest.pipelineMode = recognitionMode;
  }
...
function normalizeRecommendRequest(body) {
  ...
  const normalizedRequest = {
    ...
    annotatorName: normalizeText(source.annotatorName),
    ...
    recognitionMode: normalizeModelText(source.recognitionMode || source.pipelineMode),
      ...
      typeof source.enableThinking === "boolean"
        ...
        : typeof source.aiOptions?.enableThinking === "boolean"
          ...
          : typeof source.aiOptions?.enable_thinking === "boolean"
            ...
            : false,
    aiOptions: normalizeAiOptions(source.aiOptions),
  };
...
function createHealthPayload() {
  ...
  return {
    ...
    notes: {
      ...
      promptOverride: "支持前端覆盖听音/比较 Prompt；为空时沿用后端默认模板。",
      batchConcurrency:
