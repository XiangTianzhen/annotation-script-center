...
- 两套默认 Prompt 都已限制：
  - `heardText / recommendedText` 必须使用简体中文。
  - 不允许输出繁体字。
  - 前端不再做推荐文本的二次繁简纠正。
...
- 前端批量动作点击页面真实“保存”按钮，采用“首批并发请求 + 每次保存完成后补发下一条”的滚动窗口；批量范围固定为当前分包从第 1 条到最后 1 条，仅跳过 `dataStatus === 2` 的已完成条目，不自动提交任务、不跨分包、不触发 `.check-area`。
