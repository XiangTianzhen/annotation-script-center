...
- `data-baker-cvpc/liuzhou-helper`：DataBaker CVPC 柳州话脚本独立接口；当前开放 `segment/health`、`segment/preview`、`ai/recommend/health`、`ai/recommend/defaults`、`ai/recommend`、`clip-cache/health`、`clip-cache/upload`、`clip-cache/files/:clipId.wav`。当前段 AI 推荐链路已改为“浏览器裁剪当前波形选中段 -> 上传 1 小时临时 WAV -> 再调用 recommend”；固定边界仍是“建议生成 + 人工确认”，不自动保存、不自动提交、不自动切下一条；真实画段写入契约仍待补采。
