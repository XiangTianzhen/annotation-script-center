...
- DataBaker CVPC 已接入 beta 专属 `柳州话脚本`：当前只在 `/app/editor/asr/` 生效，已通过页内观察桥 + 多级回退获取当前音频签名 URL；助手区当前嵌入右侧 `全局标注` 卡片并收口为连续紧凑信息区，优先插入 `.label_title_border2` 内容流，右侧只保留状态与当前音频摘要。中间 `普通话顺滑` 下方统一承载 `当前段 AI 推荐 / 未填写补 Valid / 生成画段建议 / 应用当前建议`、当前画段建议结果，以及 `音频的柳州话文本 / 音频的普通话文本 / 修正后的柳州话文本` 三张 AI 推荐卡。当前段 AI 推荐严格按页面 `.xaudio_time` 的 `开始 / 结束` 裁剪当前选中段音频，浏览器端上传 1 小时 clip-cache 临时 URL 后再调用 staged AI recommend；缺少可信当前段时间时会直接阻断，不退回整段识别。前后端都固定为“建议生成 + 人工确认”，不自动保存、不自动提交、不自动切下一条，真实画段写入契约仍待补采。
...
- 平台：Alibaba LabelX、标贝易采、DataBaker CVPC（柳州话脚本 beta 已接入，当前音频签名 URL 通过页内观察桥 + 多级回退获取；右侧已收口为 `全局标注` 内 `.label_title_border2` 的紧凑信息区，中间 `普通话顺滑` 下方统一承载画段建议和三结果 AI 推荐卡；当前段 AI 推荐走 clip-cache 临时音频 URL，画段写入契约仍待补采）、Magic Data ANNOTATOR、Abaka AI（Task21助手：快捷键、AI 辅助填写、Prompt 规则、列表页统计入口）、Aishell Tech（闽南语助手已接入，当前业务能力仅在 `/mytask/mark` 生效）。
- 当前 CSV 对接字段口径：
