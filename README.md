...
- DataBaker CVPC 已接入 beta 专属 `柳州话脚本`：当前只在 `/app/editor/asr/` 生效，已通过页内观察桥 + 多级回退获取当前音频签名 URL，并桥接 `GET /httpapi/user/meta` 的最小用户快照。助手区当前嵌入右侧 `全局标注` 卡片并收口为连续紧凑信息区，优先插入 `.label_title_border2` 内容流，右侧只保留状态与当前音频摘要。中间 `普通话顺滑` 下方统一承载 `当前段 AI 推荐 / 未填写补 Valid / 生成画段建议 / 应用当前建议`、当前画段建议结果，以及 `修正后的柳州话文本 / 整理后的普通话文本` 两张最终结果卡；原始听音柳州话文本与 `特殊标签 / 需人工复核 / 备注 / AI 返回原始内容` 一并收口到默认折叠的 `当前段 AI 附加信息`。当前段 AI 推荐严格按页面 `.xaudio_time` 的 `开始 / 结束` 裁剪当前选中段音频，浏览器端直接转成 Base64 `audioDataUrl` 后调用现有 staged AI recommend；画段建议当前改成“浏览器端静音检测 + 后端增量补切预览”，默认按“静音 `>=0.4s`、阈值 `-40 dBFS`、前后补 `0.1s`”生成建议，并允许通过同源 `xaudio` DOM 交互把建议段直接画回页面当前波形。当前 AI 调用已接入系统管理 `AI 请求记录` 导出，并额外记录 `AI 调用使用人`、平台用户 `data.name / data.user_id` 与作业基础信息。固定边界仍是：不自动保存、不自动提交、不自动切下一条；画段应用后仍需人工点击平台 `保存`。
