...
当前扩展版本为 `1.1.2`；本仓库维护三个平台的五个脚本。AI 结果默认用于人工辅助，不会自动领取、自动审核或自动流转任务。
...
6. 只生成 `dist/annotation-script-center-v1.1.2.zip`。
