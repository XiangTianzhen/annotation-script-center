...
- Aishell Tech 当前已进入同平台双脚本互斥正式态：`闽南语助手` 继续保留 `转换 / 听音 / 比较` 三板块；新增 `越南语助手` 改为单阶段 Omni 转写，默认 `qwen3.5-omni-flash`，不接词表、不做转换/比较双阶段。`/mytask/mark` 当前会按 `platforms.aishellTech.activeScriptId` 只启用一个脚本。2026-06-11 hotfix 已补齐越南语助手统一队列结果解包、结构化错误 message 保留，以及 `/defaults` 失败时的本地完整默认值回退。
- 越南语助手补充优化（2026-06-11）：当 `识别文本` 与页面 `原始文本` 规范化后完全一致时，当前段结果区不再显示 `填入并保存当前条`，而是提示“与源文本一致，无需处理”；同时把单阶段 `预估人民币` 接入到结果卡与 AI 请求记录导出。
...
- DataBaker CVPC 柳州话近音纠错与语义修正（2026-06-11）：`listen` 当前会额外返回最多 `3` 条近音候选，`refine` 会结合页面上下文、JSON 主词表和参考 CSV 的读音/释义做保守纠正；若仍有歧义，结果会新增 `candidateAlternatives`，并在 `AI信息` 里展示 `近音候选参考`，同时强制人工复核。
- DataBaker CVPC 柳州话标签联动补强（2026-06-11）：后端当前新增高置信文本归一化，独立 `呃 / 诶 / 欸 -> #eh`、独立 `啊 -> #ah`、独立 `嗯 -> #um`、重复笑声 `呵呵 / 哈哈 / 嘿嘿 / 嘻嘻` 一类主说话人非语义声音 -> `<SPK/>`；`#hmm` 与 `<NPS/>` 继续只接受模型显式输出。`普通话顺滑` 当前会保留与口语词标签对应的纯文本语气词，但不会保留 `<SPK/> / <NPS/>` 或笑声文本；当前段带标签填入后若被页面下一次 tiptap 重绘临时清空，可按 `modelvalue` 自动恢复可视 chip。
- DataBaker CVPC 柳州话单独语气词 `<Meaningless>` 预设与标签空格修复（2026-06-11）：当前段填入与批量写回遇到“最终建议只剩 `#um / #hmm / #ah / #eh` 这类单独语气词标签和标点”时，会自动改写为 `Invalid + <Meaningless> + 空普通话顺滑`；结构化标签写入前会裁掉标签前后的多余半角空格，避免出现 `#ah 他又...` 这类残留空格。
- DataBaker CVPC 柳州话识别后自动填入（2026-06-11）：当前段识别成功后默认会自动整套填入 `标注文本 / 普通话顺滑`，并继续复用“单独语气词 -> `Invalid + <Meaningless> + 空普通话顺滑`”预设；该能力新增 `识别完成后自动填入` 开关，支持在 Options 和编辑页 AI 区即时关闭。
- DataBaker CVPC 柳州话单独语气词展示收口（2026-06-11）：命中 `Invalid + <Meaningless> + 空普通话顺滑` 预设时，字段结果卡与 `AI信息` 当前也同步显示最终应用结果，不再继续显示原始 `#um / 嗯。`；原始返回仍保留在 `AI 返回原始内容`。
- DataBaker CVPC 柳州话标签有效性联动与结构化写入稳定化（2026-06-11）：当前段 `填入标注文本 / 填入普通话顺滑 / 整卡填入 / 识别后自动填入` 写入前都会先按推荐标签检查并联动 `Valid / Invalid`；新增默认开启开关 `标签与有效性不一致时直接修正`，支持在 Options 和编辑页关闭。若同一条推荐同时出现有效标签和无效标签，或无效标签与正文并存，会直接判为冲突：当前段停止写入，批量跳过冲突段继续保存。带标签结构化写入当前优先走 `MAIN world` 组件状态桥接，再由 `modelvalue` / DOM 自愈兜底，修复 `<Meaningless>`、`#eh` 等 chip 被页面下一轮渲染回滚的问题。
- DataBaker CVPC 柳州话带标签当前段填入兜底补强（2026-06-11）：当前段 `标注文本` 带标签写入现在优先走“编辑器内顺序插入文本 + 页面原生标签按钮回放”，只有这条路不可用时才回退旧的结构化桥接直写，进一步贴近人工操作路径。
- DataBaker CVPC 柳州话 AI 价格估算与原始返回复制（2026-06-11）：`AI信息` 当前新增 `复制原始返回`，复制内容固定前缀为 `AI返回原始内容为：`；两阶段信息当前只显示 `预估人民币`，并额外显示 `总预估人民币`。价格统一读取共享配置 `config/pricing/aliyun-bailian-model-pricing.json`，当前按 `中国内地 / 华北2（北京）` 口径只覆盖 `qwen3.5-omni-plus / qwen3.5-omni-flash / qwen3.5-plus / qwen3.5-flash` 4 个模型，其余模型显示 `没有数据源`；AI 请求记录导出继续保留汇总 token 列，并新增中文表头的 `听音/文本修正` 分阶段 token 与人民币列，不再导出重复单价字段。
- AI 消耗记录全仓统一（2026-06-11）：当前所有已接入的 AI 服务都按共享价格配置 `config/pricing/aliyun-bailian-model-pricing.json` 估算人民币；默认返回统一 `cost` 对象，AI 请求记录 CSV 一律使用中文表头，并按“总 token + 单阶段或多阶段人民币估算”口径导出。缺少价格配置的模型继续保留调用结果，但页面只显示 `没有数据源`，CSV 金额列保持空白。
- 更多前端面板人民币估算展示（2026-06-11）：`Magic Data` 双助手总结论、`DataBaker round-one` 结果卡、`Aishell 闽南语助手` 诊断区、`LabelX` 快判/转写结果卡当前都已消费统一 `cost`。单阶段面板显示一行 `预估人民币`；多阶段面板显示阶段预估与 `总预估人民币`；缺少价格源时统一显示 `没有数据源`。
...
- AI 调用 CSV 表头统一中文；后续新增或改动的 AI 调用记录默认写 `输入Token / 输出Token / 总Token`，有人民币估算时同时写金额列。
- 只要前端已有 AI 结果信息区且结果已带统一 `cost`，默认补人民币估算展示：单阶段显示 `预估人民币`，多阶段显示阶段预估与 `总预估人民币`。
