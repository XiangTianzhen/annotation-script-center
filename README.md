...
- DataBaker CVPC 已接入 beta 专属 `柳州话脚本`：当前只在 `/app/editor/asr/` 生效，已通过页内观察桥 + 多级回退获取当前音频签名 URL；悬浮窗会展示当前音频文件、来源和可点击音频链接，完整签名地址默认折叠，页面刷新后优先消费页面真实 `annotation/meta` 响应、顶层页面或同源 `xaudio` iframe 初始化阶段打印或请求到的真实音频 URL；支持当前音频画段建议、当前段 AI 推荐和实验性字段填入；前后端都固定为“建议生成 + 人工确认”，不自动保存、不自动提交、不自动切下一条，真实画段写入契约仍待补采。
...
- 平台：Alibaba LabelX、标贝易采、DataBaker CVPC（柳州话脚本 beta 已接入，当前音频签名 URL 通过页内观察桥 + 多级回退获取，并在悬浮窗展示，画段写入契约仍待补采）、Magic Data ANNOTATOR、Abaka AI（Task21助手：快捷键、AI 辅助填写、Prompt 规则、列表页统计入口）、Aishell Tech（闽南语助手已接入，当前业务能力仅在 `/mytask/mark` 生效）。
- 当前 CSV 对接字段口径：
