...
- 系统管理中的“后端设置”当前改为显式保存：切换服务器 / 本机和修改 `AI 调用使用人` 只保留草稿，点击“保存后端设置”后才写入本地缓存并同步到侧栏摘要。
- 左侧固定侧栏当前常显 `AI 调用使用人` 只读摘要；未填写时显示“未设置”。
- 脚本详情页当前统一收口为左右双栏：左侧放脚本业务设置与快捷键，右侧放 AI 参数区或统一运行说明卡。
