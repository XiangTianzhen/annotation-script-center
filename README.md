...
- Aishell Tech 已完成独立闽南语助手首版接入：`/mytask/mark` 支持当前条 AI 推荐与批量串行真实保存，后端已注册 `/api/aishell-tech/minnan-helper/ai/recommend*` 独立接口；当前已收口为唯一的“三文本对照”策略：`two_stage + audio_first_reference + qwen3.5-omni-flash + qwen3.5-plus`，默认链路为 `POST /jobs` + 轮询，Aishell 继续保留自己的独立队列与 `success/data/meta` 契约。
