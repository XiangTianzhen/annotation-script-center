...
- 公开脚本中心本轮已改成更明确的功能页样式：
  ...
  - 平台域名标签改为可点击入口，按平台根域名在新标签页打开
  - 脚本卡改为“启停 / 详情 / 默认平台入口”紧凑功能卡，并把中部文案区改成“项目备注 / 当前功能”视图
...
- `?view=admin&tab=overview` 当前只请求系统总览接口，仪表盘默认每 `60` 秒自动刷新一次，也支持顶部“刷新数据”手动刷新。
- 仪表盘当前只保留一个可视模块：
  - 模型池占用
- 为避免高日志量场景下系统管理后台 OOM，仪表盘已暂停失败摘要、趋势、调用排行、脚本统计和运行日志展示，只保留统一 queue 的模型池占用快照。
