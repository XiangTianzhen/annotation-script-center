...
- 平台：Alibaba LabelX、标贝易采、DataBaker CVPC（柳州话脚本 beta 已接入，当前音频签名 URL 与 `user/meta` 最小用户快照通过页内观察桥 + 多级回退获取；右侧已收口为 `全局标注` 内 `.label_title_border2` 的紧凑信息区，中间 `普通话顺滑` 下方统一承载画段建议和两张最终 AI 结果卡，原始听音柳州话文本移入默认折叠的附加信息；当前段 AI 推荐走 Base64 `audioDataUrl` staged 链路，并已接入系统管理 `AI 请求记录` 导出；画段建议当前改成“前端只传 `audioUrl + 阈值`，后端 Python 直接生成整音频 preview-only 建议段”，仍需人工点击平台 `保存`）、Magic Data ANNOTATOR、Abaka AI（Task21助手：快捷键、AI 辅助填写、Prompt 规则、列表页统计入口）、Aishell Tech（闽南语助手已接入，当前业务能力仅在 `/mytask/mark` 生效）。
- 当前 CSV 对接字段口径：
