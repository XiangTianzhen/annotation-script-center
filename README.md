...
- options 当前拆成两层结构：
  ...
  - `系统管理`：进入时需要输入密码；统一承载后端设置、下载中心和已合并运行统计的系统仪表盘。
- 公开脚本中心本轮已改成更明确的功能页样式：
  ...
  - 脚本卡改为“启停 / 详情 / 默认平台入口”紧凑功能卡，不再展示每张卡重复 URL
...
- `?view=admin&tab=overview` 当前会并行请求系统总览与运行日志接口，仪表盘默认每 `60` 秒自动刷新一次，也支持顶部“刷新数据”手动刷新。
- 仪表盘当前已合并原“运行统计”页内容，统一展示：
  - 模型池占用
  - 今日调用 / 失败摘要
  - 近 14 天调用趋势
  - 近 14 天调用人排行
  - 脚本级调用统计（今日 + 最近 14 天）
  - 最近运行日志
- 为避免高日志量场景下系统管理后台 OOM，仪表盘统计当前默认只聚合“今日 + 最近 14 天”窗口，不再在页面刷新时扫描全量历史 AI 调用日志。
- 脚本详情页当前统一收口为左右双栏：左侧放脚本业务设置、启停与快捷键，右侧放 AI 参数区；没有 AI 参数的平台才回退到统一说明卡。
...
- “脚本下载中心”仍保留在系统管理的“下载中心”页签，同时公开首页和系统管理页右上角主按钮都直接打开外部下载中心。
