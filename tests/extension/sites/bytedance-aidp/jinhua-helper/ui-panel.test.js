...
const modulePath = resolveRepo("extension", "sites", "bytedance-aidp", "jinhua-helper", "ui-panel.js");

function loadUiPanelModule() {
  delete require.cache[modulePath];
  delete globalThis.ASREdgeBytedanceAidpJinhuaUiPanel;
  require(modulePath);
  return globalThis.ASREdgeBytedanceAidpJinhuaUiPanel;
}
...
test("AIDP jinhua ui panel keeps current-media and AI sections collapsed by default and shows audio plus video", function () {
...
test("AIDP jinhua current-media copies exact audio and video URLs and hides unavailable video action", async function () {
  const harness = createHarness();
  const previousDocument = globalThis.document;
  const previousHTMLElement = globalThis.HTMLElement;
