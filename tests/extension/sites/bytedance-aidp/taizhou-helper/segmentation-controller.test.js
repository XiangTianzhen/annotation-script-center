...
function loadModule() {
  ...
  delete globalThis.ASREdgeBytedanceAidpTaizhouSegmentation;
...
test("AIDP segmentation controller forwards audio, duration, existing segments, and incremental scope", async function () {
  ...
  try {
    const runtime = moduleApi.createRuntime({
      endpoint: "https://script.example.com/api/bytedance-aidp/taizhou-helper/segment/preview",
    ...
    assert.equal(
      request.url,
      "https://script.example.com/api/bytedance-aidp/taizhou-helper/segment/preview"
    );
