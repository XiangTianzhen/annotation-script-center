...
test("AIDP taizhou ui panel keeps current-media, recording-result and AI sections collapsed by default", function () {
...
test("AIDP taizhou ui panel renders read-only recording status, text and audio without writeback controls", function () {
  ...
  try {
    ...
    const panelRoot = findMountedPanelRoot(harness.body);
    const recordingCollapseButton = findNode(panelRoot, function (node) {
      return (
        node.tagName === "BUTTON" &&
        node.textContent.includes("展开当前录音平台结果")
      );
    });
    runtime.renderRecordingResult({
      sourceItemId: "source-item-1",
...
test("AIDP taizhou ui panel renders original listenText and AI return copy action", async function () {
  const harness = createHarness();
  const previousDocument = globalThis.document;
  const previousHTMLElement = globalThis.HTMLElement;
  const previousNavigator = globalThis.navigator;
