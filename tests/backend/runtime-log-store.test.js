...
function createTempRuntimeDataDir(t) {
  const runtimeDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "asc-runtime-log-store-"));
...
test("runtime log store writes daily files, removes expired files, and returns latest entries first", function (t) {
  clearRuntimeLogsForTest();
...
test("runtime log summary reports 24 hour counters and latest failure", function (t) {
  clearRuntimeLogsForTest();
  const runtimeDataDir = createTempRuntimeDataDir(t);
  configureRuntimeLogStoreForTest({
    runtimeDataDir,
...
test("runtime log store tests remove their temporary data directories", function () {
