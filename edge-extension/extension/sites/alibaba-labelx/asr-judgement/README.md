...
- ASR 文本差异高亮：对“两个ASR文本”中的 `asr_text1` 与 `asr_text2` 做字符级或词级 diff，用颜色标出新增、缺失、替换和仅标点差异，帮助标注员快速定位不同点。
- 差异摘要：在题卡内显示“完全相同 / 仅标点不同 / 字数差异 / 多处差异”等短标签，减少重复阅读成本。
- 差异导航：一题内如果差异较多，提供跳转到上一个 / 下一个差异的轻量按钮或快捷键。
- 文本布局增强：将两条 ASR 文本改成更容易对齐阅读的双行或双列显示，保留原文内容，不改变 LabelX 原始数据。
- 选择后辅助流转：可选地在选择 `1~5` 后自动滚动到下一题、自动播放下一条音频，仍不自动提交。
