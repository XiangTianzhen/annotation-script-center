...
- 成功后的页面收口当前再补一项：
  - `应用当前建议` 成功后会自动刷新当前编辑页
  - 这样无论本次走的是平台 `save_increment` 直写，还是增量 DOM 画段回退，都能立即回到最新页面状态
- UI 与文档口径同步更新：
...
- 本轮补充收口：
  - 词表处理 Prompt 改成“单份词表逐个交给网页端 AI”口径，不再假设一次处理整仓词表
  - 如果业务词表 JSON 缺失但本地参考 CSV 仍存在，运行时继续按无词表模式正常返回；前端改为在页面右下角弹出一次“没有字词对应表”提示，停留约 1 秒后自动消失
  - 如果本地连参考 CSV 也不存在，则不额外警告
- 本轮同步更新：
  - `AGENTS.md`
  - `README.md`
  - `docs/README.md`
...
- `DataBaker CVPC / 柳州话脚本` 当前继续优化表单区层级与视觉：
  ...
  - 本轮继续补 UI 细节：`未填写补 Valid / 应用当前建议` 两个次按钮当前改成橙色强调，字段内 AI 结果卡进一步强化蓝色卡片样式；`当前段 AI 附加信息` 当前默认折叠，并新增 `AI 返回原始内容` 的完整 JSON 展示。
  - 后续跟进补了按钮可见性问题：`当前段 AI 推荐 / 生成画段建议` 当前也改成与 `未填写补 Valid` 相同的橙色按钮样式，避免蓝色主按钮在页面里继续发白。
  - 后续跟进补了按钮可见性问题：上述两个次按钮当前进一步改成橙色实底 background，避免只改边框和文字色仍然不够醒目。
