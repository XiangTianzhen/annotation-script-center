...
## 2026-07-07
- 重构(ByteDance AIDP): 将脚本详情页 AIDP 下拉统一收口为可传 `options / placeholder` 的自定义组件，并删除顶部试验性 demo
...
- 优化(ByteDance AIDP): 统一金华/苏州设置页二轮交互，收口全局问号浮层、滑块状态文案、快捷键录制常驻提示、右栏 AI/快捷键替换和秒单位超时输入
...
- 优化(options): 调整共享下拉关闭策略为“菜单内部滚动不关闭，真实页面滚动或 `resize` 才关闭”，并在切脚本与 AI 面板重渲染前继续安全收口浮层
- 美化(options): 重做脚本详情页下拉触发器、菜单列表与细滚动条样式，显式隔离全局 `button` 规则，去掉灰色药丸按钮感并贴合现有详情页卡片体系
- 测试(options): 补充共享下拉关于菜单内部滚动保持展开、滚动容器滚动关闭的回归断言
- 回收(options): 功能面板首页重新对齐旧版单舞台布局、平台卡片节奏与按钮文案，并恢复平台区块“编辑顺序”拖动排序，继续写回 `settings.meta.publicCenterPlatformOrder`
- 回收(options): 系统概况 / 后端设置 / 数据导出 / 脚本下载中心 改回旧版壳层、页签结构、工具条、摘要卡片和字段排布，Vue 层仅继续复用现有 service / store 数据契约
- 回收(options): 脚本详情页按旧版“金华话脚本”模板重做两栏工作台，恢复旧版顶部信息区、启停/保存操作区、问号帮助浮层与 `基础设置 / AI 设置 / 快捷键` 三段结构
- 恢复(options): beta 隐藏入口、口令解锁和退出内测逻辑回到旧版口径，仅 beta release channel 可解锁 beta 脚本、beta 后端模式与退出内测按钮，public build 不开放 beta 能力
- 回收(options): Vue options 全站重新挂回旧版 `workspace-shell + workspace-sidebar` 左侧栏主壳层，功能面板 / 下载中心 / 系统管理 / 脚本详情统一复用品牌区、当前视图、AI 调用使用人和运行概况侧栏
- 补齐(options): 将脚本下载中心版本选择与系统管理数据导出 3 个下拉全部接入共享自定义下拉，统一旧版 trigger / 菜单 / hover / 滚动条口径，日期输入继续保持原生
- 收口(options): 系统管理 `后端设置` 恢复默认展开的 `服务器 / 本机 / Beta` 三地址配置卡，并把 `AI 调用使用人` 从后端保存链路中移回全局侧边栏，避免保存后端地址时覆盖侧栏里最新的使用人
...
## 2026-07-11
- 发布(1.0.0): 将柳州话、苏州话、金华话、杭州话四个原试用脚本正式化为唯一公开脚本集合
- 清理(extension): 删除其余 11 个脚本运行时、旧平台 shared、解锁入口、试用后端模式与双通道 build meta
- 清理(backend): 删除退役平台路由、项目数据下载命名空间和旧审计数据，统一模型响应与安全模块迁移到中性目录
- 收口(options): 脚本中心只展示三平台四脚本，系统管理只保留 Server / Local、公开下载中心和四脚本 AI 日志导出
- 发布(build): manifest 升级为 `1.0.0`，发布脚本改为单一公开 profile，产物固定为版本化 CRX/ZIP 与两份更新元数据
- 文档(readme): 同步项目、扩展、配置、平台索引与统一后端当前契约；旧过程继续保留在本历史总账
- 优化(options): 四脚本 Vue 详情页恢复运行时完整设置，统一问号帮助、双栏分组、字段顺序和响应式控件样式
- 优化(options): 平台地址拆分域名与路径显示，移除侧栏、返回按钮和平台地址胶囊的文字下划线
- 新增(options): 接入四个现有 defaults 接口与本地回退，保存时清理等于后端默认值的 Prompt/生成参数 override，并保护未保存草稿
- 修复(storage): 保留 AIDP 两阶段生成参数，支持 CVPC `value` 静音单位，并迁移杭州旧模型/识别枚举
