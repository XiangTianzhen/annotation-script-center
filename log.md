...
- 测试(shujiajia): 覆盖严格标注页 URL、默认与边界延迟、首题/后续题、重复上下文、延迟前不画段、切题与开关取消、到期 URL 复核、手动入口不受门禁及画段后自动识别。
- 修复(shujiajia): “进入新条目后自动划整段”不再要求平台创建新波形节点，改为等待扩展 AI 识别结果框的挂载代次高于上一题，并确认结果框唯一、已连接、未隐藏、可见且尺寸有效后，再检查音频时长与波形宽度；首题只建立代次基线，10 秒未重挂仍严格取消。
- 测试(shujiajia): 覆盖旧结果框不放行、同一节点断开重挂、上下文前已重挂、幂等挂载、隐藏/断连/零尺寸/重复节点、首题延迟基线、超时、切题取消及结果框后波形等待。
...
- 优化(shujiajia): AI 错误诊断增加中文“错误说明”，原始摘要改标“上游原文”，复制内容继续保留完整脱敏 JSON。
- 修复(shujiajia): 数加加 Qwen 失败响应补充上游 HTTP 状态、业务错误码、错误摘要和脱敏原始响应；JSON 响应体先结构化再按敏感字段清理，单个文本字段最多保留 20000 字符。
- 优化(shujiajia): 泸州话结果区增加可复制的 AI 错误诊断；失败保留已有识别文本，后续成功或切题清除旧诊断。
- 测试(shujiajia): 覆盖后端错误透传与脱敏上限、扩展错误 payload、页面诊断展示/复制以及失败、成功、切题状态流转。

## 2026-09-04

- 优化(shujiajia): 泸州话 AI 从听写加整理两阶段收口为单次 Omni 音频识别，规范结果字段改为 `dialectText` 并保留 `refinedText` 兼容别名；Defaults、Token、耗时、费用和调用日志统一按单阶段统计。
- 优化(shujiajia): 解除识别前必须零段画整段的限制，零段、单段和多段均可对同题音频识别；默认自动填入仅在唯一可编辑段落时执行，人工“填入转写”严格定位唯一选中行。
- 新增(shujiajia): 增加 `[OVERLAP/]` 与 `[/OVERLAP]` 两项空默认快捷键，仅调用 `Category1（多选）` 中唯一可见可用的原生符号控件，成功后标记待暂存且不自动暂存或提交。
- 测试(shujiajia): 补齐单阶段后端契约、任意段数识别、单段自动填入、选中第二段人工填入、OVERLAP 原生点击、快捷键编辑框放行及 Options/Storage 兼容回归。

## 2026-09-03

- 新增(shujiajia): storage schema 升至 `40`，数加加“识别完成后自动填入”默认开启并覆盖按钮、快捷键及画段后自动识别入口；自动填入仅作用于同题结果，失败时保留结果且不自动暂存或提交。快捷键新增空默认的“整音频划一段”，继续复用零段落可信画段与共享冲突校验。
- 新增(shujiajia): storage schema 升至 `39`，新增默认关闭的“进入新条目后自动划整段”和“助手画段成功后自动识别”；首次条目只建立上下文基线，后续切题在 iframe 等待波形就绪后每题最多尝试一次，画段成功后只等待同题音频并展示 AI 结果，不自动填入、暂存或提交；保留“填入识别结果”快捷键。
- 发布(1.2.0): 发布数加加泸州话整段识别助手稳定版本，包含可信整段划分、两阶段识别、零段/有段兼容的常驻结果面板、execute 音频对题与脱敏失败状态；不自动暂存或提交。
- 修复(shujiajia): execute 响应不再被假定含有 `taskId`，改为请求查询参数 `taskId` 与响应 `data.detail.dataId` 对题后在 MAIN world 获取音频；失败仅发送上下文与脱敏状态码，不传递资源地址、文件名、查询参数或底层异常。
- 优化(shujiajia): 助手兼容零段与已有段落布局，控制区和常驻结果区在正式容器缺失时使用父级回退，Vue 生成 `.tabs-container/.transfer` 后自动迁回；移除展开与关闭结果动作，保留人工填入。
...
- 优化(shujiajia): storage schema 升至 `38`，数加加仅保留“识别整段、填入识别结果”两项 AI 快捷键，迁移时清理播放、画段、有效性、保存和提交六项旧键位；扩展版本保持 `1.1.8`。
- 验证(shujiajia): 当前授权页面的可信 `Shift + 拖拽` 实际生成单段 `【0.005 4.210】`，在 `4.215S / 825px` 波形下满足一像素首尾容差；未点击保存、暂存或提交。
