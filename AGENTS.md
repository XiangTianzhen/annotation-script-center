...
- 供应商识别优先级：
  ...
  7. `未识别供应商`
- 任务名识别前需先 `decodeURIComponent`（失败回退原文）并清理空白（普通空格/Tab/换行/回车/全角空格），再做包含匹配与前缀推断。
...
- ASR 转写详情抓取并发按 `Math.floor(total/5)` 动态计算（`total` 为当前阶段待处理总数），最小 `1`、最大 `999`；详情优先 `pageSize=5000`，不足时继续分页补齐（分页上限 `999`）。
