...
- ByteDance AIDP 的苏州话 / 金华话 / 台州话三套脚本同平台互斥启用。
...
- 外部脚本向录音任务平台添加数据时，必须先读 `docs/recording-platform-integration.md`。
- 每个平台脚本必须显式接入且默认关闭；接入前单独确认人工触发位置、导入粒度、参考字段来源、目标任务和验收步骤，不得自动扩散到其他脚本。
- 录音平台机器 API Key 只能由统一后端从 Git 忽略的服务器私密配置读取；扩展、页面脚本和 `chrome.storage` 不得保存或接收该 Key。
- 当前台州话来源媒体已确认可由公网 HTTPS URL 直接访问：扩展只传参考文字和原始音视频 URL，脚本中心只做语法校验后转发录音平台，不下载、探测、上传或托管参考媒体；不得把第三方 Cookie、Authorization、Session 或完整请求头发送到服务器。
- 当前仅 ByteDance AIDP 台州话脚本已显式接入：Options 保存录音平台可见任务编号，服务器 `allowedTaskCodes` 再做第二重允许校验；脚本中心固定以 `BYTEDANCE_AIDP + AIDP ItemID` 绑定录音条目，无需管理员会话解锁。扩展本地只保存安全同步映射，结果只读展示，不得写回 AIDP。
