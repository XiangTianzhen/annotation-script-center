...
- 公网媒体可直传原 HTTPS URL；受登录态保护的媒体由浏览器取得文件字节后再交给未来托管能力，不得把第三方 Cookie、Authorization 或 Session 发送到服务器代下载。
- 当前规范不代表任一现有脚本已经具备录音平台写入、媒体托管、按钮或任务映射能力；只有实际代码、测试和脚本文档同时落地后才能声明已接入。
