...
- ASR 转写详情抓取并发按 `Math.floor(total/5)` 动态计算（`total` 为当前阶段待处理总数），最小 `1`、最大 `500`；详情优先 `pageSize=5000`，不足时继续分页补齐并在超限时明确告警，不静默漏数。
- ASR 快判统计也按相同并发规则执行：`Math.floor(total/5)`，最小 `1`，最大 `500`；快判详情仍保持 `pageSize=400` 口径，但必须按 `recordCount` 分页补齐。
...
- LabelX 转写上传流程已接入共享进度组件（`shared/progress-indicator.js`）：
  ...
  - 并发显示与实际执行并发一致；详情阶段按 `Math.floor(total/5)`，最小 `1`，最大 `500`。
