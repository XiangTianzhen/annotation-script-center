...
- 并行开发不得影响 `main` 工作区。
- 若启用并行分支开发，文件可改范围必须以当轮 Prompt 的白名单/禁止范围为准，不得越界修改。
...
- 当前不实现时间戳。
- 当前不实现说话人区分。
- 允许“当前题 AI 推荐”能力（仅供人工参考）。
- 当前不实现 AI 初稿、AI 校对或 AI 格式化。
- AI 推荐只允许手动点击按钮或快捷键填入，不自动填入。
- AI 推荐不自动保存、不自动提交、不自动领取、不自动流转。
- API Key 仅允许后端读取环境变量，前端不得保存或透出。
- 音频 URL 在日志中必须脱敏，只允许记录 hostname/摘要，不记录完整 URL。
- 保存方式以平台自动保存为准。
- 不新增自定义后端保存接口。
...
- 快判 AI 采用双模型 pipeline：听音模型先输出 heardText/有效性，再由比较模型给出“哪个 ASR 更优”建议；默认可使用当前题“上文”做语义消歧，但上文不能覆盖音频事实。
- 快判 AI 比较规则必须遵守官方 ASR 判断规范（0422 口径）并按 P0/P1/P2 分层决策：P0/P1 为硬伤、P2 为轻微差异。
- 快判 AI 中实意词、专有名词、动作词、否定词优先级高于标点和格式；heardText 仅作辅助，不能替代候选比较。
- `both_bad` 只能在两条都存在影响理解的 P0/P1 时使用；`uncertain_or_similar` 仅在两条都合格且无明显优劣时使用。
- 快判“使用上文理解”开关属于当前题运行态，不做全局 options 默认开关持久化；切换后需重新分析当前题生效。
...
- 提交信息使用中文，简明说明本轮改动目的。
- 当前项目按单人维护流程，执行类任务验证通过后按任务暗号与目标分支执行 push；只读审计、验证失败或用户明确禁止提交时不 push。
...
- ASR 转写当前是轻量工具栏 + options 轻量设置面板 + 快捷键配置 + 音频配置 + 统计上传能力，并允许“当前题 AI 推荐（人工确认填入）”；不恢复 legacy 保存/提交/批量流转能力。
- ASR 转写不做时间戳、说话人区分、AI 初稿/校对/格式化/标点、强制保存、自定义保存 payload、自动提交/领取/流转/跳转、整页受控执行、全页批量修改；AI 推荐结果仅作人工参考。
...
- 快判 AI 使用双模型（听音 + 文本比较）；上文默认当前题开启但仅作消歧，不能覆盖音频听感。
- 快判 AI 主判断依据是 `asr_text1/asr_text2`；听音、上文、Web Search 仅作辅助证据，不得反客为主。
- 快判 Web Search 默认开启且仅在 compare 阶段启用；不支持时仅移除搜索参数重试一次并记录 fallback。
- 快判 AI 半自动建议为默认强制能力：不提供前端启停开关，只允许手动触发分析与手动采用建议。
- 脚本 AI 高级设置默认隐藏，需在对应脚本详情页标题连续点击 10 次才显示；解锁状态仅当前 options 会话有效。
- ASR 类脚本（快判、转写、标贝易采、Magic Data）的 AI 设置统一复用“ASR 语音 AI 设置”隐藏部件，展示位置在脚本详情页标题下方。
- 脚本级 AI 设置相互独立：快判设置不得污染转写、标贝易采、Magic Data。
- “ASR 语音 AI 设置”面板解锁后必须读取对应后端 `defaults` 接口，展示后端默认模型、Prompt 与参数。
- 前端只保存脚本级 override；字段清空或恢复默认时不固化默认值，运行时由后端默认值生效。
- `response_format` 不在前端开放配置；前端不显示、不发送，后端固定结构化输出策略。
- ASR AI 的 thinking 开关必须显式传参：关闭传 `enable_thinking=false`，开启传 `enable_thinking=true`；不能靠省略参数表达关闭。
- 仅当上游返回 `enable_thinking` 不支持/参数无效时，后端才允许移除该参数重试一次，并记录 fallback 状态。
- 不支持的模型参数前端不显示；后端必须按白名单过滤，未支持参数不得透传模型接口。
- Magic Data 快捷键设置属于非 AI 运行控制项，必须保持常显，不纳入 AI 隐藏面板。
- 快判 AI 的建议答案文案（`answerText`）必须由后端固定映射五个选项：`第一个更好`、`第二个更好`、`不确定或差不多`、`都不好`、`其他方言或语种`；模型不得自定义建议答案文案。
- 当两条 ASR 主体语义一致但标点/空格/数字格式存在明显优劣时，应选择更规范的一条，不得一律判为“不确定或差不多”。
...
- “提交任务 / 提交任务并结束”仅允许作为快捷键动作，不加入顶部工具栏按钮。
- 提交类快捷键只能点击页面真实系统按钮，不直接请求平台 API，也不自动确认二次弹窗。
