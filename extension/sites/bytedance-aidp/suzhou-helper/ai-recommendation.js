(function () {
  ...
  const DEFAULT_PATH = "/api/bytedance-aidp/suzhou-helper/ai/recommend";
  ...
  function buildFetchFailureMessage(rawMessage) {
    ...
    return "连接苏州话 AI 后端失败，请检查 options 首页后端接口地址、后端服务状态或当前网络后重试。 " + suffix;
  }
    ...
    async function postRecommendation(source, selectedRange, audioDataUrl) {
      ...
      try {
        ...
        if (!response.ok || !payload || payload.success !== true) {
          const error = new Error(payload?.message || "苏州话 AI 识别失败。");
  ...
  globalThis.ASREdgeBytedanceAidpSuzhouAiRecommendation = api;
