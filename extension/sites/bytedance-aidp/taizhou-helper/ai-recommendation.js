(function () {
  ...
  const DEFAULT_PATH = "/api/bytedance-aidp/taizhou-helper/ai/recommend";
  ...
  function buildFetchFailureMessage(rawMessage) {
    ...
    return "连接台州话 AI 后端失败，请检查 options 首页后端接口地址、后端服务状态或当前网络后重试。 " + suffix;
  }
  ...
  function requireSelectedRange(selectedRange, selectionKey) {
    ...
    if (
      ...
      throw new Error("未读取到当前段的开始/结束时间，请先选中目标段后重试。");
    }
    return {
      ...
      durationMs: Math.max(
        0,
        Math.round(toFiniteNumber(range.durationMs, Number(range.endMs) - Number(range.startMs)))
      ),
    };
  }
  ...
  function createRuntime(options) {
    ...
    function buildRecommendBody(source, selectedRange, audioDataUrl) {
      ...
      const aiOmni = normalizeAiOmni(config.aiOmni);
      if (Object.keys(aiOmni).length > 0) {
        body.aiOmni = aiOmni;
      }
      return body;
    }
    ...
    async function postRecommendation(source, selectedRange, audioDataUrl) {
      ...
      try {
        ...
        if (!response.ok || !payload || payload.success !== true) {
          const error = new Error(payload?.message || "台州话 AI 识别失败。");
  ...
  globalThis.ASREdgeBytedanceAidpTaizhouAiRecommendation = api;
