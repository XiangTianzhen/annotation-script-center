(function () {
  if (globalThis.ASREdgeBytedanceAidpJinhuaContent) {
    if (typeof module !== "undefined" && module.exports) {
      module.exports = globalThis.ASREdgeBytedanceAidpJinhuaContent;
    }
    return;
  }
  ...
  const dataApiFactory = globalThis.ASREdgeBytedanceAidpJinhuaDataApi || null;
  const aiFactory = globalThis.ASREdgeBytedanceAidpJinhuaAiRecommendation || null;
  const segmentFactory = globalThis.ASREdgeBytedanceAidpJinhuaSegmentation || null;
  const uiFactory = globalThis.ASREdgeBytedanceAidpJinhuaUiPanel || null;
  const shortcutFactory = globalThis.ASREdgeBytedanceAidpJinhuaShortcuts || null;
  const SCRIPT_ID =
    CONSTANTS.BYTEDANCE_AIDP_JINHUA_HELPER_SCRIPT_ID || "bytedanceAidpJinhuaHelper";
  const AI_PATH = "/api/bytedance-aidp/jinhua-helper/ai/recommend";
  const SEGMENT_PREVIEW_PATH = "/api/bytedance-aidp/jinhua-helper/segment/preview";
  ...
  function getShortcutActionDefinitions() {
    return Array.isArray(CONSTANTS.BYTEDANCE_AIDP_JINHUA_SHORTCUT_ACTIONS)
  ...
  function updateSegmentRecognizeButton(button, segmentNumber, options) {
    ...
    button.setAttribute(SEGMENT_RECOGNIZE_ACTION_ATTR, "recognize");
  }
  ...
  function createBatchRecommendController(options) {
    ...
    async function runRecognition(selectedNumbers) {
      ...
      if (Array.isArray(selectedNumbers) && selectedNumbers.length === 0) {
        const message = "请先选择至少一个段落后再执行批量识别。";
  ...
  function ensureHelperRuntime(settings) {
    ...
    runWithProtectedScrollState(document, function () {
      ...
      ui.setStatus(
        "金华话脚本已就绪；当前支持单段识别直填输入框、批量识别和分段建议暂存写回。",
        "success"
      );
    });
    syncPlaybackSensitiveDecorations(
  ...
  globalThis.ASREdgeBytedanceAidpJinhuaContent = api;
  ...
  if (
    ...
    window &&
    typeof document !== "undefined" &&
    globalThis.__ASREdgeBytedanceAidpJinhuaInstalled !== true
  ) {
    globalThis.__ASREdgeBytedanceAidpJinhuaInstalled = true;
    if (isManagementPage()) {
      void installTaskListUi();
