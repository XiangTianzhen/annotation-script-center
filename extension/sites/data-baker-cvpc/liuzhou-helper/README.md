...
- 把助手区嵌入右侧 `全局标注` 卡片：原生 `Valid / Invalid` 保持在上方；助手区当前固定展示状态、当前音频/当前段摘要、按需出现的画段建议、三结果 AI 推荐卡和动作按钮
- 波形区 `.bottom-right` 当前只前置两个分段按钮：
  - `生成画段建议`
  - `应用当前建议`
- 右侧助手动作当前固定为：
  - `当前段 AI 推荐`
...
- `ui-panel.js`：右侧 `全局标注` 卡内助手挂载，展示当前运行时音频地址、选中段摘要、画段建议、三结果 AI 推荐卡和动作按钮
