...
- 字段结果卡在没有 AI 结果时不显示任何占位文案；有结果时改成“文本左侧、按钮右侧”的紧凑头部布局
...
- `ui-panel.js`：右侧 `全局标注` 卡内紧凑信息区 + `是否有效（Valid or Not）` 下方独立 AI 工作区挂载；右侧只保留状态与逐行音频摘要，字段内承载三结果 AI 推荐卡，独立 AI 区承载动作按钮、画段建议与附加信息，并统一收口为系统蓝主调样式
