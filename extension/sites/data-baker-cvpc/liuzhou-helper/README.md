...
- 生成当前音频的画段建议：
  ...
  - 如果本地已检出候选静音、但增量补切结果仍为空，前端会自动再请求一次“整条音频重切预览”作为 fallback
  - fallback 当前只用于预览：会明确显示“当前增量补切未命中，以下为整条音频重切预览”，并列出整条音频建议段
  - 当预览结果为空时，AI 区会补充说明“本地未检出静音”“未命中现有段内部”或“命中了静音但拆分后仍不足 2 段”
- `应用当前建议` 当前已接入同源 `xaudio` iframe DOM 交互：
  - 先按 live region 匹配受影响原段
  - 把原段改成第一个建议子段
  - 再点击原生 `开启拆分`，把剩余建议子段画回当前波形
  - 应用完成后只更新页面当前波形状态，不自动点平台 `保存`
  - 如果当前预览属于整条音频 fallback 或其他 `applyAllowed=false` 只读预览，运行时会直接拒绝应用
- 对当前波形选中段执行两阶段识别：
...
- 整条音频 fallback 预览当前只读，不支持一键应用；如需继续提升效果，下一阶段再单独接入手动 Python 试算
