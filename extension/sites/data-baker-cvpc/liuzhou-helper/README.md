...
- 把助手区嵌入右侧 `全局标注` 卡片：原生 `Valid / Invalid` 保持在上方；右侧当前只保留状态、当前音频/当前段摘要和提示说明，并优先插入 `全局标注` 内部的 `.label_title_border2` 内容流；摘要里会额外显示“当前第 N 段”
- `是否有效（Valid or Not）` 下方当前作为独立同级 AI 工作区，集中承载：
  - `当前段 AI 推荐`
  - `未填写补 Valid`
  - `生成画段建议`
  - `应用当前建议`
...
- `ui-panel.js`：右侧 `全局标注` 卡内紧凑信息区 + `是否有效（Valid or Not）` 下方独立 AI 工作区挂载；右侧只保留状态与音频摘要，字段内承载三结果 AI 推荐卡，独立 AI 区承载动作按钮、画段建议与附加信息
