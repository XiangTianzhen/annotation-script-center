...
- `词表状态与模式` 当前固定显示 `主词表状态 / 听音参考 开启|关闭 / 文本修正固定携带`；其中听音参考只跟随 options 持久开关 `附带词表参考（听音辅助）`，默认关闭。
...
- 听音阶段当前会额外返回最多 `3` 条近音候选；文本修正阶段会结合页面上下文、JSON 主词表和参考 CSV 的释义/读音做保守纠正。若仍有歧义，`AI信息` 会追加 `近音候选参考`，并强制 `需人工复核=true`
