(function () {
  ...
  const MAGIC_DATA_HANGZHOU_SCRIPT_ID =
    globalThis.ASREdgeConstants?.MAGIC_DATA_HANGZHOU_SCRIPT_ID || "magicDataHangzhouAssistant";
  const PANEL_ROOT_SELECTOR = "[data-asc-magic-data-hangzhou-review-inline]";
  const INLINE_SUGGESTION_SELECTOR = "[data-asc-magic-data-hangzhou-inline-suggestion]";
  const SPEAKER_SUGGESTION_SELECTOR = "[data-asc-magic-data-hangzhou-speaker-suggestion]";
  const RAW_MODAL_SELECTOR = "[data-asc-magic-data-hangzhou-raw-modal]";
  ...
  async function loadMagicDataSettings() {
    ...
    try {
      ...
      const projectSettings =
        ...
        settings?.projects?.magicDataHangzhouAssistant ||
        {};
      ...
      if (activeScriptId && activeScriptId !== MAGIC_DATA_HANGZHOU_SCRIPT_ID) {
        runtimeSettings.enabled = false;
        runtimeSettings.aiReviewEnabled = false;
      }
      return runtimeSettings;
  ...
  function createRuntime() {
    ...
    const aiClient = globalThis.__ASREdgeMagicDataHangzhouAiReviewClient;
    const panelFactory = globalThis.__ASREdgeMagicDataHangzhouInlinePanel;
    const shortcutsFactory = globalThis.__ASREdgeMagicDataHangzhouShortcuts;
      ...
      shortcutsRuntime.setActions({
        ...
        reviewCurrent: function () {
          if (runtimeSettings.aiReviewEnabled === false) {
            panel.setMessage("杭州话助手已在 options 中关闭。");
            return Promise.resolve();
          }
    ...
    function refresh() {
      ...
      if (pageType !== lastRouteLog) {
        ...
        safeInfo("[MagicData][Hangzhou] route detected: " + pageType);
      }
    ...
    function ensurePanelMountedWithRetry() {
      ...
      if (mountRetryCount >= MOUNT_RETRY_LIMIT) {
        safeWarn("[MagicData][Hangzhou] panel mount retry exhausted");
        return;
      }
      mountRetryCount += 1;
  ...
  async function evaluatePage() {
    ...
    try {
      ...
      if (!runtime) {
        ...
        if (!runtime) {
          safeWarn("[MagicData][Hangzhou] runtime dependencies missing");
          return;
        }
        await runtime.start();
        return;
      ...
      safeWarn("[MagicData][Hangzhou] runtime error: " + String(error?.message || error || "unknown"));
      if (runtime) {
        runtime.refresh();
      }
    } finally {
  ...
  if (!startLogged) {
    startLogged = true;
    safeInfo("[MagicData][Hangzhou] content started");
  }
