...
- `two_stage`：听音模型 + 比较模型。
- `omni_single`：单模型直接生成完整结果。

识别策略：

- `direct_dialect`：直接识别杭州话/方言文本。
- `mandarin_to_dialect`：先取得普通话文本，再转换或比较方言结果。
...
单双模型选择会动态控制相关模型字段。Prompt、temperature、top_p、max_tokens、max_completion_tokens、presence_penalty、frequency_penalty、seed 和 stop sequences 均可通过 Options 保存覆盖值。
