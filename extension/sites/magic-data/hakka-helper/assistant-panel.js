(function () {
  const ROOT_ATTR = "data-asc-magic-data-hakka-review-inline";
  const STYLE_ATTR = "data-asc-magic-data-hakka-review-inline-style";
  const INLINE_SUGGESTION_ATTR = "data-asc-magic-data-hakka-inline-suggestion";
  const SPEAKER_SUGGESTION_ATTR = "data-asc-magic-data-hakka-speaker-suggestion";
  const RAW_MODAL_ATTR = "data-asc-magic-data-hakka-raw-modal";
  ...
  const RESIZE_BODY_CLASS = "asc-magic-data-hakka-review-resizing";
  ...
  function createButton(text, className) {
    ...
    button.className = "asc-md-hakka-btn el-button el-button--mini is-plain";
    if (className) {
  ...
  function ensureStyle() {
    ...
    style.textContent = [
      ...
      "[" + ROOT_ATTR + "] .asc-md-hakka-btn.el-button{border-color:#4b5563;background:rgba(51,65,85,.42);color:#e5e7eb;}",
      "[" + ROOT_ATTR + "] .asc-md-hakka-btn.el-button:hover{background:rgba(71,85,105,.78);}",
      "[" + ROOT_ATTR + "] .asc-md-hakka-btn.el-button.el-button--primary{border-color:#2563eb;background:#2563eb;color:#f8fafc;}",
      "[" + ROOT_ATTR + "] .asc-md-hakka-btn.el-button.el-button--primary:hover{background:#1d4ed8;}",
      "[" + ROOT_ATTR + "] .asc-md-hakka-btn.el-button.el-button--success{border-color:#22c55e;background:#16a34a;color:#f8fafc;}",
      "[" + ROOT_ATTR + "] .asc-md-hakka-btn.el-button.el-button--success:hover{background:#15803d;}",
      "[" + ROOT_ATTR + "] .asc-md-hakka-btn.is-plain{background:rgba(30,41,59,.68);}",
      ...
      "[" + INLINE_SUGGESTION_ATTR + "] .asc-md-hakka-inline-text{font-size:12px;color:#e2e8f0;white-space:pre-wrap;word-break:break-word;margin-bottom:0;}",
      "[" + INLINE_SUGGESTION_ATTR + "] .asc-md-hakka-inline-row{display:flex;align-items:center;justify-content:space-between;gap:8px;min-height:24px;}",
      "[" + INLINE_SUGGESTION_ATTR + "] .asc-md-hakka-inline-row .asc-md-hakka-inline-text{margin:0;flex:1;}",
      "[" + ROOT_ATTR + "] .asc-md-hakka-diff-same{color:#e2e8f0;}",
      "[" + ROOT_ATTR + "] .asc-md-hakka-diff-insert{background:rgba(34,197,94,.22);color:#bbf7d0;padding:0 1px;border-radius:2px;}",
      "[" + ROOT_ATTR + "] .asc-md-hakka-diff-delete{background:rgba(239,68,68,.24);color:#fecaca;padding:0 1px;border-radius:2px;text-decoration:line-through;}",
      "[" + INLINE_SUGGESTION_ATTR + "] .asc-md-hakka-diff-same{color:#e2e8f0;}",
      "[" + INLINE_SUGGESTION_ATTR + "] .asc-md-hakka-diff-insert{background:rgba(34,197,94,.22);color:#bbf7d0;padding:0 1px;border-radius:2px;}",
      "[" + INLINE_SUGGESTION_ATTR + "] .asc-md-hakka-diff-delete{background:rgba(239,68,68,.24);color:#fecaca;padding:0 1px;border-radius:2px;text-decoration:line-through;}",
      ...
      "[" + SPEAKER_SUGGESTION_ATTR + "] .asc-md-hakka-speaker-text{font-size:12px;color:#e5e7eb;}",
      "[" + SPEAKER_SUGGESTION_ATTR + "] .asc-md-hakka-speaker-actions{display:flex;gap:6px;margin-top:4px;flex-wrap:wrap;}",
      ...
      "[" + RAW_MODAL_ATTR + "] .asc-md-hakka-raw-dialog{width:min(920px,96vw);max-height:86vh;display:flex;flex-direction:column;border:1px solid #334155;border-radius:8px;background:#0f172a;color:#e5e7eb;}",
      "[" + RAW_MODAL_ATTR + "] .asc-md-hakka-raw-head{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:10px;border-bottom:1px solid #334155;}",
      "[" + RAW_MODAL_ATTR + "] .asc-md-hakka-raw-title{font-size:13px;font-weight:700;color:#dbeafe;}",
      "[" + RAW_MODAL_ATTR + "] .asc-md-hakka-raw-actions{display:flex;gap:6px;}",
  ...
  function createRuntime(deps) {
    ...
    function renderDiffParts(container, parts) {
      ...
      (Array.isArray(parts) ? parts : []).forEach(function (part) {
        ...
        span.className = "asc-md-hakka-diff-" + (part.type || "same");
    ...
    function openRawOutputModal() {
      ...
      dialog.className = "asc-md-hakka-raw-dialog";
      const head = document.createElement("div");
      head.className = "asc-md-hakka-raw-head";
      const title = document.createElement("div");
      title.className = "asc-md-hakka-raw-title";
      title.textContent = "AI 原始输出（脱敏）";
      const actions = document.createElement("div");
      actions.className = "asc-md-hakka-raw-actions";
    ...
    function renderSpeakerSuggestion(formItem, labelText, checkData, validateFn) {
      ...
      let text = wrapper.querySelector(".asc-md-hakka-speaker-text");
      if (!(text instanceof HTMLElement)) {
        text = document.createElement("div");
        text.className = "asc-md-hakka-speaker-text";
    ...
    function ensureInlineSuggestionBlock(container, type, taskKey) {
      ...
      row.className = "asc-md-hakka-inline-row";
      const text = document.createElement("div");
      text.className = "asc-md-hakka-inline-text";
    ...
    function updateInlineSuggestionBlock(block, type, checkData) {
      ...
      const row = block.querySelector(".asc-md-hakka-inline-row") || block;
      const textNode = row.querySelector(".asc-md-hakka-inline-text");
      if (textNode) {
    ...
    function ensureMounted() {
      ...
      sub.textContent = "以平台现有文本为基准，AI 仅做客家话规则质检和风险提示，不自动保存、不自动提交。";
    ...
    return {
      ...
      triggerToggleDialectDetail: function () {
        return toggleFoldSection("客家话内容");
      },
