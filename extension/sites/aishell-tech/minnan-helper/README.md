...
- “当前识别结果”区会额外显示：原始文本、原始文本先按 `minnan-lexicon.csv + minnan-lexicon-rules.json` 严格生成的闽南语候选文本、听音文本、推荐文本，以及“听音文本 vs 词表转写文本”的差异高亮。
...
- 当前仅保留 `音频优先，文本参考`：
  ...
  - 后端会先按 `minnan-lexicon.csv` 和 `minnan-lexicon-rules.json` 纯代码生成标准闽南语候选 `lexiconCandidateText`，再只聚焦 `heardText` 与候选文本的差异项。
