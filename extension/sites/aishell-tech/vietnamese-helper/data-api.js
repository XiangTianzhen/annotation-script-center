(function () {
  if (globalThis.__ASREdgeAishellTechVietnameseDataApiInstalled === true) {
    return;
  }
  globalThis.__ASREdgeAishellTechVietnameseDataApiInstalled = true;
  ...
  function normalizeMarkCompareText(text) {
    return normalizeVietnameseTranscriptionText(text || "");
  }

  function collapseVietnameseSpaces(text) {
    return String(text || "").replace(/[\s\u3000]+/g, " ").trim();
  }

  function normalizeVietnameseTranscriptionText(text) {
    const value = collapseVietnameseSpaces(text);
    if (!value) {
      return "";
    }
    ...
    return /[.!?…]/.test(last) ? normalized : normalized + ".";
  }
  ...
  function doesRenderedItemMatch(expected) {
    ...
    const referenceMatched =
      ...
      normalizeVietnameseTranscriptionText(currentReferenceText) ===
      normalizeVietnameseTranscriptionText(expectedReferenceText);
  ...
  globalThis.__ASREdgeAishellTechVietnameseDataApi = api;
})();
