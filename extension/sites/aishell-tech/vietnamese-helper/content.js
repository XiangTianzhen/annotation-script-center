(function () {
  if (globalThis.__ASREdgeAishellTechVietnameseContentInstalled === true) {
    return;
  }
  globalThis.__ASREdgeAishellTechVietnameseContentInstalled = true;
  ...
  async function loadRuntimeConfig() {
    ...
    const settings = STORAGE && typeof STORAGE.getSettings === "function"
      ...
      : { platforms: {}, meta: {} };
    const scriptConfig = settings?.platforms?.aishellTech?.scripts?.vietnameseHelper || {};
    const endpoint =
  ...
  function createRuntime(config) {
    const dataApiFactory = globalThis.__ASREdgeAishellTechVietnameseDataApi;
    const aiFactory = globalThis.__ASREdgeAishellTechVietnameseAiRecommendation;
    const diagnosticsFactory = globalThis.__ASREdgeAishellTechVietnameseDiagnostics || {};
    const uiFactory = globalThis.__ASREdgeAishellTechVietnameseUiPanel;
    const shortcutsFactory = globalThis.__ASREdgeAishellTechVietnameseShortcuts;
    ...
    function start() {
      ...
      panel.setStatus("越南语助手已就绪。", "success");
    }
    ...
    routeTimer = window.setInterval(function () {
      if (location.href !== currentUrl) {
        ...
        void evaluatePage().catch(function (error) {
          console.warn("[Aishell][vietnamese-helper] route evaluate failed", error?.message || error);
        });
      }
    }, 300);
  }
  ...
  function bootstrap() {
    startRouteWatch();
    void evaluatePage().catch(function (error) {
      console.warn("[Aishell][vietnamese-helper] bootstrap failed", error?.message || error);
    });
  }
