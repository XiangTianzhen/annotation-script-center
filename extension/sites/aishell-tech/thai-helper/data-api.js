(function () {
  if (globalThis.__ASREdgeAishellTechThaiDataApiInstalled === true) {
    return;
  }
  globalThis.__ASREdgeAishellTechThaiDataApiInstalled = true;
  ...
  function normalizeMarkCompareText(text) {
    return normalizeThaiTranscriptionText(text || "");
  }

  function collapseThaiSpaces(text) {
    return String(text || "").replace(/[\s\u3000]+/g, " ").trim();
  }

  function normalizeThaiTranscriptionText(text) {
    const value = collapseThaiSpaces(
      String(text || "")
        .replace(/[，、]/g, ",")
        .replace(/[。｡]/g, ".")
        .replace(/[？]/g, "?")
  ...
  function extractSavedMarkFields(result) {
    ...
    if (source.mark && typeof source.mark === "object") {
      return {
        ...
        speed: normalizeThaiSpeedValue(source.mark.speed || ""),
      };
    }
    if (typeof source.mark === "string") {
      ...
      if (parsed && typeof parsed === "object") {
        return {
          ...
          speed: normalizeThaiSpeedValue(parsed.speed || ""),
        };
      }
      return {
        text: normalizeText(source.mark),
        speed: "",
      };
    }
    return {
      text: normalizeText(source.text || ""),
      speed: normalizeThaiSpeedValue(source.speed || ""),
    };
  }
  ...
  function buildSaveShortMarkPayload(item, value, options) {
    ...
    const normalizedText = normalizeThaiTranscriptionText(payloadValue.text || "");
    const normalizedSpeed = normalizeThaiSpeedValue(payloadValue.speed || "");
    return {
      mark: JSON.stringify({
        text: normalizedText,
  ...
  function doesRenderedItemMatch(expected) {
    ...
    const referenceMatched =
      ...
      normalizeThaiTranscriptionText(currentReferenceText) ===
      normalizeThaiTranscriptionText(expectedReferenceText);
  ...
  function buildItemFromRecord(record, routeParams, taskDetail, options, fallbackIndex) {
    ...
    function fillPageFields(value) {
      ...
      const nextText = normalizeThaiTranscriptionText(source.text);
      const nextSpeed = normalizeThaiSpeedValue(source.speed);
    ...
    async function clickSaveAndWait(options) {
      ...
      const expectedSpeed = normalizeThaiSpeedValue(options?.expectedSpeed || "");
      ...
      while (Date.now() < deadline) {
        ...
        if (now - networkCheckAt >= 450) {
          ...
          try {
            ...
            const textMatched =
              !expectedText ||
              normalizeMarkCompareText(savedFields.text) === expectedText;
            const speedMatched =
              !expectedSpeed ||
              normalizeThaiSpeedValue(savedFields.speed) === expectedSpeed;
  ...
  const api = {
    ...
    doesRenderedItemMatch,
    normalizeThaiTranscriptionText,
    normalizeThaiSpeedValue,
  ...
  globalThis.__ASREdgeAishellTechThaiDataApi = api;
})();
