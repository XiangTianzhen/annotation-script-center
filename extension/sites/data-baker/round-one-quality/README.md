...
- `shortcuts.js`：监听 DataBaker 专属快捷键；普通输入不拦截，命中已配置快捷键时会先退出输入焦点再执行，不绕过 disabled 按钮。
...
- “填入推荐文本”成功后会自动退出“本句话文本”输入框，方便继续使用快捷键；不会自动保存、自动提交或自动判定。
...
14. 聚焦“本句话文本”输入框，输入未配置为快捷键的普通字符，确认正常输入且不被拦截。
15. 聚焦“本句话文本”输入框后按已配置快捷键，例如 `Alt+A`，确认输入框失焦且快捷键执行。
16. 点击“填入推荐文本”，确认只写入“本句话文本”输入框，且输入框自动失焦；不会自动保存、提交、合格、不合格或任务判定。
17. 当后端返回的推荐文本没有句末标点时，确认推荐卡和填入后的输入框末尾都有中文句末标点。
18. 填入后不点击页面，直接按快捷键，确认仍可继续响应。
19. 关闭 DataBaker 脚本后刷新详情页，确认工具卡、自动分页和快捷键都停止；只关闭 AI 推荐时不显示工具卡。
20. 打开非 `roundOneCollect` 页面，确认不注入该工具卡。
