...
- 左侧句子列表上方 `filter-screen` 新增“AI填入合格项”按钮（位于“批量判定”按钮右侧；挂载失败时回退到 AI 面板内）。
- 点击该按钮会先刷新当前页列表，再只处理 `statusName=质检合格` 的下一条，自动选中该条、生成 AI 推荐并填入文本；每次只处理一条。
...
- “AI填入合格项”会跳过 `质检不合格` 和 `未质检`，不点击 checkbox，不自动保存、不自动提交、不做批量流转。
