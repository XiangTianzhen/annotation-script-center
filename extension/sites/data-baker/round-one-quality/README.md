...
- 点击后读取当前题的 `audioUrl`、页面候选文本、句子编号、朗读要求、有效时间和音频时长，再调用本地 Node 后端。
...
1. 重新加载扩展。
2. 启动本地后端：`node platform-resources\backend\server.js`。
3. 访问 health 接口，确认返回 `success=true`。
4. 打开 `https://datafactory.data-baker.com/v2/#/quality/roundOneCollect?collectId=...&checkType=0`。
5. 等待列表和右侧题目加载，确认右侧“本句话文本”下方出现 `DataBaker AI 推荐文本` 工具卡。
6. 点击左侧不同句子，确认旧推荐结果会清空或提示需要重新生成。
7. 点击“AI 推荐文本”，确认只请求当前选中单条。
8. `DATABAKER_AI_MOCK=1` 时，确认结果卡能展示 mock 推荐文本。
9. 点击“复制推荐文本”，确认剪贴板内容为推荐文本。
10. 点击“填入推荐文本”，确认只写入“本句话文本”输入框；不会自动保存、提交、合格、不合格或任务判定。
11. 刷新页面、切换题目和翻页后，确认不残留上一题结果卡。
12. 打开非 `roundOneCollect` 页面，确认不注入该工具卡。
