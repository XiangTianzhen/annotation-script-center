...
- AI连续填入合格项并发数量默认 `20`，范围 `1~50`；前端并发越高，后端排队越多，但不会绕过限流。
...
- `two_stage + qwen3.5-omni-plus / qwen3.5-omni-flash`：默认优先走 Omni legacy 快速路径，先通过 Qwen Omni `input_audio` 得到 `heardText`，再调用 compare 模型生成 `recommendedText`
- `omni_single + qwen3.5-omni-plus / qwen3.5-omni-flash`：当 `DATABAKER_AI_OMNI_LEGACY_FAST_PATH=1` 时，也优先切到 Omni legacy 快速路径兜底，以保证基础速度和稳定性
