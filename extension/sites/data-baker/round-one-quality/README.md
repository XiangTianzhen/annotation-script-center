...
36. 点击“AI并发分析并连续填入合格项”，在 `fun-asr` 下确认默认并发为 `25`、范围 `1~50`；切到 Omni 后确认默认并发为 `15`、范围 `1~25`，填入仍保持顺序消费。
...
- 批量流程仍保持：直接 recommend 请求、`30ms` 错峰；前端并发按模型动态归一，Omni 默认 `15` / `1~25`，Fun-ASR 默认 `25` / `1~50`。
...
- 当前前端仍保持直接 `POST /ai/recommend`、`30ms` 错峰；前端并发按模型动态归一，且不默认走 `/jobs`。
