...
- 数加加泸州话只在当前零段落时允许通过受限可信 `Shift + 拖拽` 将整音频划为一段，按两个波形像素验收且失败时保留段落、不自动 Delete；自动入口以新 `taskId:dataId` 为触发信号，并只在 iframe 的 `document.referrer` 是合法数加加标注页时按可配置固定延迟（默认 2500ms）单次执行，首题与后续切题均适用，不再等待网络资源、结果框重挂或波形轮询。识别本身不再要求先画段，零段、单段和多段均可对同题音频执行一次 Omni 泸州方言识别。控制区与常驻结果区优先挂载 `.tabs-container`、`.transfer`，零段状态使用父级回退并在正式容器出现后自动迁回。默认自动填入仅在恰好一个可编辑段落时执行；人工“填入转写”严格写入唯一 `.el-table__row.current-row`。整段画段、识别、填入及两个 OVERLAP 原生符号操作共五项快捷键默认均为空；OVERLAP 动作可在转写输入框聚焦时触发，并只点击 `Category1（多选）` 的原生控件。扩展只标记待暂存，不自动设置有效性、暂存或提交。
