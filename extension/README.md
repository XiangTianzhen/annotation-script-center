...
- 平台摘要中的域名标签当前已改成可点击入口：按平台 `matches` 的第一个 URL 模式推导根域名，并在新标签页打开。
- 脚本卡中部文案区当前统一改成“项目备注 / 当前功能”视图：
  - 优先展示 `script.note`
  - `note + description` 同时存在时分成“项目备注 + 当前功能”两段
  - 只有 `description` 时自动回退为单段说明
- 系统管理页统一承载后端设置、下载中心与已合并运行统计的系统仪表盘，并要求进入时输入密码。
...
- `options/options.html` 保持单入口，但当前路由固定为：
  ...
  - `?view=admin&tab=overview|backend|downloads`
- `公开脚本中心` 默认直接进入，只保留：
  - 平台概览
  - 脚本启停
  - 脚本详情入口
...
- “项目数据下载 / AI 请求记录导出”统一迁到 `?view=admin&tab=downloads`。
- “系统仪表盘”当前只展示“模型池占用”，并默认每 `60` 秒自动刷新一次；顶部“刷新数据”按钮可立即手动刷新。
- 为控制高日志量环境下的内存占用，仪表盘已暂停失败摘要、趋势、调用排行、脚本统计与运行日志展示，只保留统一 queue 的模型池占用快照。
- 模型池卡片当前统一改为中文状态视图，展示“正在处理 / 等待处理 / 池容量 / 剩余可接收”，并按“当前占用 / 当前空闲 / 后端池已满”解释状态，不再直接暴露 `活跃 x / y · 排队 z` 这类技术文案。
- `?view=admin&tab=downloads` 当前除了项目数据下载 / AI 请求记录导出外，还内置“扩展版本下载”面板：
  - 默认显示最新版
  - 历史版本使用下拉框选择
  - 当前选中版本按存在性展示 `CRX` 和可选 `ZIP` 下载按钮
  - 版本数据来自后端 `GET /api/admin/download-center/releases`
- 公开首页与系统管理页右上角主按钮当前都进入系统管理内的“下载中心”页签；下载面板中仍保留“打开外部目录”作为兜底入口。
...
- ASR 类脚本（快判、转写、标贝易采、Magic Data、Aishell）统一复用右侧“ASR 语音 AI 设置”工作区：默认直接展示，仅影响当前脚本，配置独立保存；左侧只保留业务设置、启停与快捷键，不支持参数前端不显示、后端不发送。
