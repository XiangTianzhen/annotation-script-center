...
`extension/` 是当前正式 `1.1.8` Chrome / Edge Manifest V3 扩展成品目录。该目录可以直接作为 unpacked extension 加载，也是 ZIP 打包脚本的唯一输入。
...
- ISOLATED world：零分段可信整段划分、两阶段 AI、原生区域内嵌面板、两项 AI 快捷键和原生写入编排
...
- 数加加泸州话只在当前零段落时允许通过受限可信 `Shift + 拖拽` 将整音频划为一段，按两个波形像素验收且失败时保留段落、不自动 Delete；音频使用 execute 请求查询参数的 `taskId` 与响应 `data.detail.dataId` 对题，完整资源地址仅在 MAIN world 临时使用。控制区与常驻结果区优先挂载 `.tabs-container`、`.transfer`，零段状态使用父级回退并在正式容器出现后自动迁回。识别结果须人工点击填入，填入后标记待暂存。扩展不自动设置有效性、不自动暂存、不自动提交；平台已有的播放、画段、有效性、保存和提交快捷操作不再由扩展代理。
...
使用 `dist/annotation-script-center-v1.1.8.zip` 时，需要先解压，再加载解压后的扩展根目录。
