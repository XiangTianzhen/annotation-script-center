...
- “系统仪表盘”当前同时展示模型池、失败摘要、趋势、调用排行、脚本统计与运行日志，并默认每 `60` 秒自动刷新一次；顶部“刷新数据”按钮可立即手动刷新。
- 为控制高日志量环境下的内存占用，仪表盘统计当前只聚合“今日 + 最近 14 天”窗口；旧的全量累计不再在刷新时同步扫描。
- 公开首页和系统管理页右上角主按钮当前都直接打开外部“脚本下载中心”；脚本详情页右上角则继续进入系统管理。
