...
- 数加加泸州话只在当前零段落时允许通过受限可信 `Shift + 拖拽` 将整音频划为一段，按两个波形像素验收且失败时保留段落、不自动 Delete；两个默认关闭的开关可在首次条目后的切题中自动划整段，并在助手画段成功后等待同题音频自动识别。音频使用 execute 请求查询参数的 `taskId` 与响应 `data.detail.dataId` 对题，完整资源地址仅在 MAIN world 临时使用。控制区与常驻结果区优先挂载 `.tabs-container`、`.transfer`，零段状态使用父级回退并在正式容器出现后自动迁回。识别结果须人工点击按钮或使用快捷键填入，填入后标记待暂存。扩展不自动设置有效性、不自动暂存、不自动提交；平台已有的播放、画段、有效性、保存和提交快捷操作不再由扩展代理。
