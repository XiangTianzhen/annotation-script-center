...
- 数加加泸州话只在当前零段落时允许通过受限可信 `Shift + 拖拽` 将整音频划为一段，按两个波形像素验收且失败时保留段落、不自动 Delete；页面初始化期间先在 MAIN world 内存暂存 execute 的最小音频快照，脚本确认启用后立即消费，避免异步设置读取漏掉首个响应。控制区位于 `.tabs-container` 的有效性区域下方，结果区位于 `.transfer` 的原生转写区下方。识别结果须人工点击填入，填入后标记待暂存。扩展不自动设置有效性、不自动暂存、不自动提交；平台已有的播放、画段、有效性、保存和提交快捷操作不再由扩展代理。
