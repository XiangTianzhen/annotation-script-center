...
`extension/` 是当前正式 `1.1.6` Chrome / Edge Manifest V3 扩展成品目录。该目录可以直接作为 unpacked extension 加载，也是 ZIP 打包脚本的唯一输入。
...
使用 `dist/annotation-script-center-v1.1.6.zip` 时，需要先解压，再加载解压后的扩展根目录。
