...
- ByteDance AIDP 苏州、金华、台州辅助面板统一使用“当前媒体信息”，从当前 Receive 条目展示音频和视频地址，不再展示模板；缺少视频时显示“无视频”。
- 金华话、台州话均使用单次 Qwen Omni：后端把字符串原始输出逐字符映射为响应 `listenText`。`listenText` 仅是扩展/API 兼容字段，而非模型 JSON 字段，不做 JSON 解析、文本清洗或解释提取。
- 台州话“基础设置”可填写录音平台数据库内部 `taskId`；配置后详情页在“清空画段”左侧显示“添加数据”，将 Search Item 的完整题目作为一条录音数据导入，并在默认折叠的“当前录音平台结果”中只读显示状态、完成文本和音频。机器 Key 只在服务器，媒体登录头和原始签名 URL 不会传给脚本中心。
