...
- 台州话“基础设置”可填写录音平台数据库内部 `taskId`；配置后详情页在“清空画段”左侧显示“添加数据”，将 Search Item 的完整题目作为一条录音数据导入，并在默认折叠的“当前录音平台结果”中只读显示状态、完成文本和音频；来源题目、录音条目、状态与手动刷新位于同一响应式信息栏。机器 Key 只在服务器，媒体登录头和原始签名 URL 不会传给脚本中心。
