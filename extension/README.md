...
- ISOLATED world：零分段可信整段划分、两阶段 AI、右下悬浮面板、两项 AI 快捷键和原生写入编排
...
- 数加加泸州话只在当前零段落时允许通过受限可信 `Shift + 拖拽` 将整音频划为一段，按两个波形像素验收且失败时保留段落、不自动 Delete；音频从 execute 响应的已确认存储地址在页面内存中读取。识别结果须人工点击填入，填入后标记待暂存。扩展不自动设置有效性、不自动暂存、不自动提交；平台已有的播放、画段、有效性、保存和提交快捷操作不再由扩展代理。
