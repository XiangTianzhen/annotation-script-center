...
- 台州话“基础设置”可填写录音平台数据库内部 `taskId`；配置后详情页在“清空画段”左侧显示“添加数据”，将 Search Item 的参考文字及公网 HTTPS 音视频 URL 作为一条录音数据导入，并在默认折叠的“当前录音平台结果”中只读显示状态、完成文本和音频；来源题目、录音条目、状态与手动刷新位于同一响应式信息栏。扩展不下载参考媒体，机器 Key 只在服务器，AIDP Cookie、Authorization 和 Session 不会传给脚本中心。
