...
- `?view=admin&tab=backend` 当前改为显式保存工作流：切换后端模式和修改 `AI 调用使用人` 只更新当前草稿，点击“保存后端设置”后才写入本地缓存。
- 左侧侧栏当前常显 `AI 调用使用人` 摘要，方便确认当前全局 AI 调用身份。
...
- ASR 类脚本（快判、转写、标贝易采、Magic Data、Aishell）统一复用右侧“ASR 语音 AI 设置”工作区：默认直接展示，仅影响当前脚本，配置独立保存；不支持参数前端不显示、后端不发送。
