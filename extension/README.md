...
- 台州话详情页的“全自动导入并押后”是当前任务范围内的 DOM 自动化例外：只有用户手动开始才运行，状态为 `AVAILABLE` 后只点击文本精确为“押后”的可用原生按钮，在标题精确为“押后原因”的唯一可见弹层中填写 `1` 并点击同一按钮组的“确定”。每步最多等 20 秒；任何歧义、失败或未验证进入下一题都会停止，不会点击“提交”、领取或调用 AIDP 接口。停止会在确认前取消该弹层，确认后不再触发下一步；该内存态流程不写 storage、不改 Options、后端或 manifest。
